<%@ Control CodeBehind="ListView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="Taoqi.InTransitCheck.ListView" %>


<div class="row nav2">
    <div class="col-md-8 listTitle">在途气发布审核</div>
        ...
        <Taoqi:SplendidGrid ID="grdMain" SkinID="grdListView" PageSize="15"
            ...
            <Columns>
                <asp:TemplateColumn HeaderText="操作" ItemStyle-Width="160px">
                    <ItemTemplate>
                        <asp:LinkButton runat="server" ID="btnEdit" Visible='<%# Eval("C_Status").ToString() == "0" %>' OnClientClick="return confirm('提示：确认通过吗？');" CommandName="Pass" CommandArgument='<%# Eval("ID") %>'>
                            <div class="btnOrange">审核通过</div>
                        </asp:LinkButton>
                        <asp:LinkButton runat="server" ID="btnDelete" Visible='<%# Eval("C_Status").ToString() == "0" %>' OnClientClick="return confirm('提示：确认不予通过吗？');" CommandName="NoPass" CommandArgument='<%# Eval("ID") %>'>

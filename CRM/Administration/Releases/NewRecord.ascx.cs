	...
	public class NewRecord : NewRecordControl
	{
		...
		public override void ValidateEditViewFields()
		{
			if ( !IsEmpty() )
			{
				...
				// 10/20/2011   Apply Business Rules to NewRecord. 
			...
			try
			{
				if ( e.CommandName == "NewRecord" )
				{
					...
					if ( Page.IsValid )
					{
						...
						// 02/21/2010   An error should not forward the command so that the error remains. 
						// In case of success, send the command so that the page can be rebuilt. 
						// 06/02/2010   We need a way to pass the ID up the command chain. 

...
function RELEASES_RELEASE_NAME_Changed(fldRELEASE_NAME)
{
	...
	else if ( fldPREV_RELEASE_NAME.value != fldRELEASE_NAME.value )
	{
		if ( fldRELEASE_NAME.value.length > 0 )
		{
			try
			{
				Taoqi.Releases.AutoComplete.RELEASES_RELEASE_NAME_Get(fldRELEASE_NAME.value, RELEASES_RELEASE_NAME_Changed_OnSucceededWithContext, RELEASES_RELEASE_NAME_Changed_OnFailed, userContext);
			}
			catch(e)
			{
...
function RELEASES_FOUND_IN_RELEASE_Changed(fldFOUND_IN_RELEASE)
{
	...
	else if ( fldPREV_FOUND_IN_RELEASE.value != fldFOUND_IN_RELEASE.value )
	{
		if ( fldFOUND_IN_RELEASE.value.length > 0 )
		{
			try
			{
				Taoqi.Releases.AutoComplete.RELEASES_RELEASE_NAME_Get(fldFOUND_IN_RELEASE.value, RELEASES_FOUND_IN_RELEASE_Changed_OnSucceededWithContext, RELEASES_FOUND_IN_RELEASE_Changed_OnFailed, userContext);
			}
			catch(e)
			{
...
function RELEASES_FIXED_IN_RELEASE_Changed(fldFIXED_IN_RELEASE)
{
	...
	else if ( fldPREV_FIXED_IN_RELEASE.value != fldFIXED_IN_RELEASE.value )
	{
		if ( fldFIXED_IN_RELEASE.value.length > 0 )
		{
			try
			{
				Taoqi.Releases.AutoComplete.RELEASES_RELEASE_NAME_Get(fldFIXED_IN_RELEASE.value, RELEASES_FIXED_IN_RELEASE_Changed_OnSucceededWithContext, RELEASES_FIXED_IN_RELEASE_Changed_OnFailed, userContext);
			}
			catch(e)
			{

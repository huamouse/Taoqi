...
namespace Taoqi.Administration.Releases
{
	...
	public class PopupView : SplendidControl
	{
		...
		// 02/21/2010   Controls to manage inline create. 
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				// 02/21/2010   Handle new events that hide and show the NewRecord panel. 
				...
				{
					// 08/03/2010   After creating the record, automatically select it. 
					ScriptManager mgrAjax = ScriptManager.GetCurrent(this.Page);
					if ( mgrAjax != null )
					{
						Guid   gID   = Sql.ToGuid(e.CommandArgument);
						string sNAME = Crm.Modules.ItemName(Application, m_sMODULE, gID);
						// 08/17/2012   Terminate the javascript line. 
					...
					{
						// 02/21/2010   Redirect instead of rebind so that the entire page gets reset. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// 06/30/2009   Visibility is already controlled by the ASPX page, but it is probably a good idea to skip the load. 

...
namespace Taoqi.Administration.RenameTabs
{
	/// <summary>
	...
	public class ListView : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// 09/08/2005  An empty key is valid, so use a separate INSERT field. 
			if ( txtRENAME.Value == "1" )
			{
				try
				{
					...
					// 01/17/2006   Also need to clear the TabMenu. 
					SplendidCache.ClearTabMenu();
					// 04/20/2006   Also clear the term for the list. 
					L10N.SetTerm(ctlSearch.LANGUAGE, String.Empty, "moduleList", txtKEY.Value, txtVALUE.Value);
					txtRENAME.Value = "";
					// 09/09/2005   Transfer so that viewstate will be reset completely. 
					// 01/04/2005   Redirecting to default.aspx will loose the language setting.  Just rebind. 
				}
				catch(Exception ex)
				{
					SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);

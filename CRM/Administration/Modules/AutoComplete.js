...
function MODULES_MODULE_NAME_Changed(fldMODULE_NAME)
{
	...
	else if ( fldPREV_MODULE_NAME.value != fldMODULE_NAME.value )
	{
		if ( fldMODULE_NAME.value.length > 0 )
		{
			try
			{
				Taoqi.Administration.Modules.AutoComplete.MODULES_MODULE_NAME_Get(fldMODULE_NAME.value, MODULES_MODULE_NAME_Changed_OnSucceededWithContext, MODULES_MODULE_NAME_Changed_OnFailed, userContext);
			}
			catch(e)
			{
...
function MODULES_MODULE_NAME_Changed_OnSucceededWithContext(result, userContext, methodName)
{
	if ( result != null )
	{
		...
		// 08/31/2009   We want to automatically click the update button. 

	...
	public class ListView : SplendidControl
	{
					...
					case "Update":
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									// 07/25/2010   If hte name has changed, then we need to delete the old and insert the new. 
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				// 09/18/2012   Provide quick access to the create option. 
				...
				else if ( e.CommandName == "Dropdown.MoveUp" )
				{
					...
					// 09/08/2005   If the list changes, reset the cached values. 
					SplendidCache.ClearList(ctlSearch.LANGUAGE, ctlSearch.DROPDOWN);
					//TERMINOLOGY_BindData(true);
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						// 01/16/2006   New lists should go directly to list editing. 
						string sDROPDOWN = Sql.ToString(Request.QueryString["DROPDOWN"]);
						// 11/19/2005   The language must be initialized before the search clause is applied. 
						if ( !IsPostBack )
						{
							...
							// 09/18/2012   Allow direct access to language. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				// 07/25/2010   Lets experiment with jQuery drag and drop. 
				...
				bEnableAdd = (Taoqi.Security.AdminUserAccess(m_sMODULE, "edit") >= 0);
				if ( !IsPostBack )
				{
					grdMain.ShowFooter = bEnableAdd;
				...
				else
				{
					...
					// 07/25/2010   The ImageButtons are working without binding. 
					// 07/25/2010   The problem with the ReorderList commands not firing was that the OnCommand was still set to Page_Command. 
					TERMINOLOGY_BindData(false);
				}
			}
			catch(Exception ex)
			{
				// 01/20/2006   Need to catch all errors.  Saw a dropdown error when creating a new dropdown. 
				lblError.Text = ex.Message;
				SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);
#if DEBUG

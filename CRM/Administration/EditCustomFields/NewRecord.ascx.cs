...
namespace Taoqi.Administration.EditCustomFields
{
	...
	public class NewRecord : SplendidControl
	{
		...
		public string MODULE_NAME
		{
			...
			{
				sMODULE_NAME = value;
				// 01/06/2008   HeaderLeft replaced the header to this control.
				// 05/17/2010   Change to ListHeader now that NewRecord has been moved below the ListView. 
				ctlListHeader.Title = L10n.Term("EditCustomFields.LBL_ADD_FIELD") + " " + L10n.Term(".moduleList." + sMODULE_NAME);
				// 02/13/2007   Web1.4 is having a problem displaying this control.  
		...
		public void Clear()
		{
			...
			// 03/18/2010   We are having trouble hiding the NewRecord control. 
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "NewRecord" )
			{
				...
				// 01/11/2006   The label does not need to be validated because it will become the term display name. 
				...
				if ( Page.IsValid )
				{
					...
					try
					{
						// 01/11/2006   The label needs to be stored in the TERMINOLOGY table. 
						// 05/20/2007   The Label term is no longer used.  The label term must be derived from the field name 
						...
						// 04/24/2006   We don't support MassUpdate at this time. 
						...
						// 01/11/2006   Add term to the local cache.  Always default to english. 
						//Application["en-US." + sMODULE_NAME + "." + sLABEL_TERM] = txtLABEL.Text;
						// 04/05/2006   A _C is appended to the term in the procedure.  Do so here as well. 
						// 05/20/2007   The label is also prepended with LBL_.  This is a requirement for the reporting system. 
						// 05/20/2007   The Label term is no longer used.  The label term must be derived from the field name 
						// in order for the reporting area to work properly.  The reporting area assumes that the label is the of the format "LBL_" + Name + "_C". 

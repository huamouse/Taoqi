...
namespace Taoqi.Calendar
{
	...
	public class MyCalendar : DashletControl
	{
		...
		{
			// 08/31/2006   The date needs to be separated into day, month, year fields to avoid localization issues. 
			Response.Redirect("~/Calendar/default.aspx?" + CalendarControl.CalendarQueryString(ctlCalendar.SelectedDate));
		}

		private void Page_Load(object sender, System.EventArgs e)
		{
			// 09/09/2007   We are having trouble dynamically adding user controls to the WebPartZone. 
			...
			// 07/10/2009   The end-user will be able to hide or show the Dashlet controls. 
			/*
			if ( this.Visible && !Sql.IsEmptyString(sDetailView) )
			{
				// 01/17/2008   We need to use the sDetailView property and not the hard-coded view name. 

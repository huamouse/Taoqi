			...
			try
			{
				switch ( e.CommandName )
				{
					case "Day.Previous":
					{
						...
						// 06/15/2006   There seems to be a conflict with binding the DayGrid inside InitializeComponent(). 
						...
						try
						{
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								using ( DataTable dt = new DataTable() )
								{
									...
									// 04/30/2009   The hour range should be config values so that the range can be extended. 
									...
									for(int iHour = nHourMin ; iHour <= nHourMax ; iHour++ )
									{
										...
										// 10/21/2008   Protect against an invalid date. 
										...
										// 06/09/2006   Fix so that a 1 hour meeting does not span two hours.  DATE_END should not allow DATE_END = HOUR_START. 
...
//										RegisterClientScriptBlock("vwACTIVITIES_List" + dtHOUR_START.ToOADate().ToString(), Sql.EscapeJavaScript(vwMain.RowFilter));
		...
		private void InitializeComponent()
		{
			...
			// 06/09/2006   If this is a postback, then build the grid early so that we will not lose any button events. 
			if ( IsPostBack )
			{
				// 06/09/2006   T10n has not been defined at this point, so create it. 
				GetT10n();
				BindGrid();
			}
		}

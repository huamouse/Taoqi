<%@ Control CodeBehind="ListView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="Taoqi.Administration.Languages.ListView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divListView">
	...
	<Taoqi:ModuleHeader ID="ctlModuleHeader" Module="Administration" Title="Administration.LBL_MANAGE_LANGUAGES" EnablePrint="true" HelpName="index" EnableHelp="true" Runat="Server" />
	...
	<Taoqi:SplendidGrid id="grdMain" AllowPaging="false" AllowSorting="false" EnableViewState="true" runat="server">
		<Columns>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="10%" ItemStyle-Wrap="false">
				<ItemTemplate>
					<asp:ImageButton Visible='<%# Taoqi.Security.AdminUserAccess(m_sMODULE, "edit") >= 0 && (Sql.ToString(Eval("NAME")) != "en-US") &&  Sql.ToBoolean(Eval("ACTIVE")) %>' CommandName="Languages.Disable" CommandArgument='<%# Eval("NAME") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term("Administration.LNK_DISABLE") %>' SkinID="minus_inline" Runat="server" />
					<asp:LinkButton  Visible='<%# Taoqi.Security.AdminUserAccess(m_sMODULE, "edit") >= 0 && (Sql.ToString(Eval("NAME")) != "en-US") &&  Sql.ToBoolean(Eval("ACTIVE")) %>' CommandName="Languages.Disable" CommandArgument='<%# Eval("NAME") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term("Administration.LNK_DISABLE"         ) %>' Runat="server" />
					<asp:ImageButton Visible='<%# Taoqi.Security.AdminUserAccess(m_sMODULE, "edit") >= 0 && (Sql.ToString(Eval("NAME")) != "en-US") && !Sql.ToBoolean(Eval("ACTIVE")) %>' CommandName="Languages.Enable"  CommandArgument='<%# Eval("NAME") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term("Administration.LNK_ENABLE" ) %>' SkinID="plus_inline" Runat="server" />
					<asp:LinkButton  Visible='<%# Taoqi.Security.AdminUserAccess(m_sMODULE, "edit") >= 0 && (Sql.ToString(Eval("NAME")) != "en-US") && !Sql.ToBoolean(Eval("ACTIVE")) %>' CommandName="Languages.Enable"  CommandArgument='<%# Eval("NAME") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term("Administration.LNK_ENABLE"          ) %>' Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# Taoqi.Security.AdminUserAccess(m_sMODULE, "delete") >= 0 && (Sql.ToString(Eval("NAME")) != "en-US") %>' CommandName="Languages.Delete" CommandArgument='<%# Eval("NAME") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_DELETE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# Taoqi.Security.AdminUserAccess(m_sMODULE, "delete") >= 0 && (Sql.ToString(Eval("NAME")) != "en-US") %>' CommandName="Languages.Delete" CommandArgument='<%# Eval("NAME") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_DELETE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>

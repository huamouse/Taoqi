...
namespace Taoqi.Administration.Configurator
{
	/// <summary>
	...
	public class AdminWizard : SplendidControl
	{
		...
		// 01/26/2014   Atlantic theme header logo. 
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
						{
							...
							Taoqi.FileBrowser.FileWorkerUtils.LoadImage(ref gImageID, ref sFILENAME, UPLOAD_LOGO_IMAGE.UniqueID, trn);
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
						{
							Guid   gImageID  = Guid.Empty;
							string sFILENAME = String.Empty;
							Taoqi.FileBrowser.FileWorkerUtils.LoadImage(ref gImageID, ref sFILENAME, UPLOAD_ATLANTIC_IMAGE.UniqueID, trn);
							if ( !Sql.IsEmptyGuid(gImageID) )
							{
		...
		protected void SaveSystemName()
		{
			...
			// 07/07/2010  Use the last-run to determine if the AdminWizard should be run. 
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					// 07/29/2010   The Mail Settings is the last page, so we will get a Continue event and not a Next event. 
				...
				else if ( e.CommandName == "Smtp.Test" )
				{
					...
					// 07/29/2010   If the password is provided, then we need to save it as the text field will be cleared in the response. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			this.Visible = Taoqi.Security.isAdmin;
			if ( !this.Visible )
			{
				Response.Redirect("~/Home/");
				return;
			...
			if ( !IsPostBack )
			{
					...
					{
						// 02/23/2009   Allow the logo to be any URL. 
						...
						// 08/09/2009   Allow the image to be relative to the application. 
						...
						// 11/27/2008   Company logo is a config value, not a term. 
						// 08/18/2010   IE8 does not support alt any more, so we need to use ToolTip instead. 
						imgCompanyLogo.ToolTip = Sql.ToString(Application["CONFIG.company_name"]);
					}
					else
					{
						HEADER_LOGO_IMAGE .Text = "~/Include/images/Taoqi_Logo.gif";
					...
					if ( Sql.IsEmptyString(ATLANTIC_HOME_IMAGE.Text) )
						ATLANTIC_HOME_IMAGE.Text = "~/Include/images/Taoqi_Icon.gif";
					...
					try
					{
						...
						{
							// 07/09/2010   Normalize the GUID so that there will not be a case significant issue. 
							// 08/19/2010   Check the list before assigning the value. 
					...
					// 01/08/2008   If the encryption key does not exist, then we must create it and we must save it back to the database. 
					// 01/08/2008   SugarCRM uses blowfish for the inbound email encryption, but we will not since .NET 2.0 does not support blowfish natively. 
					Guid gINBOUND_EMAIL_KEY = Sql.ToGuid(Application["CONFIG.InboundEmailKey"]);

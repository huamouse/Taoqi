...
namespace Taoqi.Administration.BusinessRules
{
	...
	public class ExportSQL : SplendidPage
	{
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			this.Visible = (Taoqi.Security.GetUserAccess("BusinessRules", "export") >= 0);
			if ( !this.Visible )
				return;
							...
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
								{
									if ( rdr.Read() )
									{
										...
										// 09/12/2012   Assigned User ID and Team ID are not used when migrating. 
										gASSIGNED_USER_ID = Guid.Empty;
										gTEAM_ID          = Guid.Empty;
										sTEAM_SET_LIST    = String.Empty;

										// 01/23/2012   Remove some of the bulk of the XML. 

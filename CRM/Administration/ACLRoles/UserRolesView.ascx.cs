			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Roles.Remove":
					{
						...
						// 05/03/2006   Don't redirect so that the selected user will not change. 
						//Response.Redirect("RolesByUser.aspx");
						// 05/03/2006   We do have to rebind after the modification. 
						BindGrid();
						// 12/07/2006   We need to rebind the access view after changing the roles. 
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 05/03/2006   Always bind, so that we don't have to redirect to show changes. 
								//if ( !IsPostBack )
								{

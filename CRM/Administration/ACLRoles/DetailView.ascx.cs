	...
	public class DetailView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
				...
				else if ( e.CommandName == "FieldSecurity" )
				{
					Response.Redirect("FieldSecurity/default.aspx?ID=" + gID.ToString());
				}
			}
			catch(Exception ex)
			{
		...
		{
			SetPageTitle(L10n.Term(m_sMODULE + ".LBL_ROLE"));
			...
			try
			{
				...
				// 11/28/2005   We must always populate the table, otherwise it will disappear during event processing. 
				// 03/19/2008   Place AppendDetailViewFields inside OnInit to avoid having to re-populate the data. 
				if ( !IsPostBack )
				{
							...
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										{
											// 11/25/2006   If item is not visible, then don't show its sub panel either. 

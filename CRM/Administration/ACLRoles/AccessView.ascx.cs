...
namespace Taoqi.Administration.ACLRoles
{
	/// <summary>
	...
	public class AccessView : SplendidControl
	{
		...
		{
			// 12/07/2006   We need to be able to force the grid to be rebound when its data has changed. 
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							// 04/26/2006   Normally, we would only bind if not a postback, 
							...
							if ( bAllowAdminRoles )
							{
								...
								grdACL_Admin.DataSource = vwMain ;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				...
				if ( !IsPostBack )
				{
					...
					if ( !Sql.IsEmptyGuid(gUSER_ID) )
					{
						if ( pnlAdmin.Visible )
						{
							pnlAdmin.Visible = Sql.ToBoolean(Application["CONFIG.allow_admin_roles"]) && (Taoqi.Security.AdminUserAccess("Users"   , grdACL_Admin.EnableACLEditing ? "edit" : "view") >= 0);
							// 03/17/2010   Only display admin roles if the user is an Admin Delegate. 
							DbProviderFactory dbf = DbProviderFactories.GetFactory();
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								con.Open();
					...
					{
						pnlAdmin.Visible = Sql.ToBoolean(Application["CONFIG.allow_admin_roles"]) && (Taoqi.Security.AdminUserAccess("ACLRoles", grdACL_Admin.EnableACLEditing ? "edit" : "view") >= 0);
					}
				}
				BindGrid();
			}
			...
			// 05/03/2006   Remove the page data binding as it clears the binding on UserRolesView.ascx. 
		}

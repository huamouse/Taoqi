...
namespace Taoqi.Administration.ACLRoles
{
	...
	public class Users : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					// 07/20/2010   Don't need both the add and the append. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			this.Visible = (Taoqi.Security.AdminUserAccess("ACLRoles", "list") >= 0);
			if ( !this.Visible )
				return;
			...
			if ( !IsPostBack )
			{
				...
				//Page.DataBind();
				ctlDynamicButtons.Visible = !PrintView && (Taoqi.Security.AdminUserAccess("ACLRoles", "edit") >= 0);

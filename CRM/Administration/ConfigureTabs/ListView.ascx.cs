...
namespace Taoqi.Administration.ConfigureTabs
{
	/// <summary>
	...
	public class ListView : SplendidControl
	{
		...
		protected void grdMain_ItemCreated(object sender, DataGridItemEventArgs e)
		{
			...
			else if ( e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem )
			{
				...
				if ( row != null )
				{
					if ( !((Sql.ToBoolean(row["TAB_ENABLED"]) || Sql.ToBoolean(row["TAB_ENABLED"])) && (Taoqi.Security.AdminUserAccess(m_sMODULE, "edit") >= 0)) )
						e.Item.CssClass += " nodrag nodrop";
				}
			}
		}
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				// 10/24/2009   ClearTabMenu() does all necessary clearning. 
				//Cache.Remove("vwMODULES_TabMenu");
				// 06/03/2006   The tab menu is now user-specific, but we will only clear the current user. 
				SplendidCache.ClearTabMenu();
				TERMINOLOGY_BindData(true);
				// 08/21/2007   The easiest way to rebuild the current menu is to refresh the page. 
				// 10/02/2007   Use an AJAX UpdatePanel to reduce whole page repaints. 
		...
		private void TERMINOLOGY_BindData(bool bBind)
		{
			...
			try
			{
				...
				{
					string sSQL;
					// 08/22/2007   Admin modules cannot be displayed as a tab. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			TERMINOLOGY_BindData(true);
			// 01/04/2006   DataBind seems to be required, otherwise the table header will not get translated. 

...
namespace Taoqi.Administration.DynamicLayout.EditViews
{
	...
	public class EditView : DynamicLayoutView
	{
		...
		protected override void LayoutCopy(string sOldLayoutViewName, string sNewLayoutViewName, IDbTransaction trn)
		{
			...
			// 02/14/2013   Just in case the user tries to copy over to a layout that already exists, we need to delete that layout first. 
			...
			// 02/14/2013   Now copy all the relationships. 
			sSQL = "select *                               " + ControlChars.CrLf
			...
			// 02/14/2013   Now copy all the buttons. 
			sSQL = "select *                                  " + ControlChars.CrLf
		...
		{
			tblMain.Rows.Clear();
			// 09/02/2012   Now that we are creating multiple tables, we need to remove the dynamic tables. 
			...
			if ( dtFields != null )
			{
				// 09/12/2009   A previous pass may have hidden the table, so we need to reset the visibility state. 
				tblMain.Visible = true;
				DataView dv = dtFields.DefaultView;
				dv.RowFilter = "DELETED = 0";
				dv.Sort      = LayoutIndexName();
				// 04/11/2011   Allow the layout mode to be turned off to preview the result. 
				...
				// 07/27/2010   Hide the border when nothing is displayed. 
		...
		protected override void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
					...
					if ( ctlNewRecord != null )
					{
						...
						if ( vwFields.Count == 1 )
						{
							foreach(DataRowView row in vwFields)
							{
								...
								// 09/12/2009   Add FIELD_VALIDATOR_ID. 
					...
					if ( ctlNewRecord != null )
					{
						...
						if ( vwFields.Count == 1 )
						{
							...
							foreach(DataRowView row in vwFields)
							{
								...
								// 01/20/2006   Need to duplicate the the LIST_NAME in the CACHE_NAME as the dynamic code uses CACHE_NAME. 
						...
						{
							// 01/08/2006   If not found, then insert a new field. 
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								// 01/09/2006   Mark existing records for deletion. 
								// This is so that the save operation can update only records that have changed. 
								foreach(DataRow row in dtFields.Rows)
									row["DELETED"] = 1;
								da.Fill(dtFields);
								// 01/09/2006   We need to change the IDs for two reasons, one is to prevent updating the Default Values,
				...
				// 04/11/2011   Update the view on PreviewChanged. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// 08/02/2010   Hide the Export button on the Community Edition. 
			if ( !IsPostBack )
			{

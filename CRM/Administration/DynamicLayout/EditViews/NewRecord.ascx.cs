...
namespace Taoqi.Administration.DynamicLayout.EditViews
{
	/// <summary>
	...
	{
		// 01/19/2010   Add support for new DATA_FORMAT field. 
		...
		// 09/12/2009   Add FIELD_VALIDATOR so that they can be disabled. 
		...
		protected TableRow           trFORMAT_SCRIPT     ;  // 09/16/2012   Unused. 
		...
		// 02/05/2008   trFORMAT_ROWS is a union of trTEXT and trLIST_NAME. 
		...
		public int COLSPAN
		{
			...
			set
			{
				// 09/30/2012   This seems like an old bug.  -1 is used with Saluation and First Name. 
				if ( value > 0 || value == -1 )
		...
		// 04/11/2011   Add support for Dynamic Layout popups.
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( rdr.Read() )
							{
								// 04/11/2011   Copy all fields except ID and index. 
		...
		// 09/12/2009   Provide default messages for common validators. 
		...
		// 05/17/2010   We need to be able to refresh the visibility flags. 
		public override void lstFIELD_TYPE_Changed(Object sender, EventArgs e)
		{
			// 02/05/2008   trFORMAT_ROWS is a union of trTEXT and trLIST_NAME. 
			// 01/19/2010   Add support for new DATA_FORMAT field. 
			// 09/16/2012   ONCLICK_SCRIPT is valid for ListBox, Radio, CheckBoxList, RelatedListBox, RelateCheckBoxList, CheckBox 
			switch ( lstFIELD_TYPE.SelectedValue )
			{
				...
				// 10/07/2010   Allow a format for a label.  Also show module type. 
				...
				// 05/17/2009   Add support for a generic module popup. 
				// 08/04/2010   DATA_FORMAT is used to store AutoComplete and UseContextKey flags. 
				...
				// 09/02/2009   Add support for ModuleAutoComplete. 
				...
				// 08/26/2009   Add support for dynamic teams. 
				...
				// 02/28/2008   Hidden field only shows general fields. 
				...
				// 09/02/2012   A separator is just like a blank. 
				// 09/16/2012   The data field can be used as the table id. 
				// 09/20/2012   Data Format will store initial visibility state. 
			...
			// 09/16/2012   Rename REQUIRED1 and REQUIRED2 to DATA_REQUIRED and UI_REQUIRED.  
			trUI_REQUIRED     .Visible = trDATA_REQUIRED    .Visible;
			// 09/16/2012   Rename CHANGE1 and CHANGE2 to DISPLAY_FIELD and ONCLICK_SCRIPT.  
			//trONCLICK_SCRIPT  .Visible = trDISPLAY_FIELD   .Visible;
			// 09/16/2012   Rename TEXT1, TEXT2 and TEXT3 to FORMAT_MAX_LENGTH, FORMAT_SIZE and FORMAT_COLUMNS. 
			trFORMAT_SIZE     .Visible = trFORMAT_MAX_LENGTH.Visible;
			trFORMAT_COLUMNS  .Visible = trFORMAT_MAX_LENGTH.Visible;
			// 09/16/2012   Rename GENERAL1, GENERAL2 and GENERAL3 to FORMAT_TAB_INDEX, COLSPAN and ROWSPAN. 

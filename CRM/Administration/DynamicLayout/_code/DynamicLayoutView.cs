...
namespace Taoqi.Administration.DynamicLayout
{
	/// <summary>
	...
	public class DynamicLayoutView : SplendidControl
	{
		...
		protected Taoqi._controls.ListHeader ctlListHeader;

		private   NewRecord               ctlNewRecord    ;
		protected HtmlTable               tblMain         ;
		// 11/30/2006   Not sure why, but we are having a problem with loading the viewstate. 
		// Store the fields in a hidden variable so that it is accessible inside Page_Init. 
		protected HtmlInputHidden         txtFieldState   ;
		// 11/30/2006   Make the dtFields member so that it only needs to be loaded once. 
		protected DataTable               dtFields = null;
		// 10/30/2010   Add support for Business Rules Framework. 
		...
		{
			dtFields = null;
			// 11/30/2006   Pull the field state directly from the request so that this will work even before viewstate has been restored. 
		...
		// 01/09/2006   Instead of creating an abstract class, just create virtual members.
		...
		// 02/14/2013   Allow a layout to be copied. 
		...
		// 01/20/2010   We need to know the name field. 
		...
		public int DynamicTableNewFieldIndex()
		{
			...
			{
				// 01/08/2006   Only count records that are not deleted. 
		...
		public void DynamicTableDelete(int nFieldIndex)
		{
			...
			{
				// 01/08/2006   Only modify records that are not deleted. 
				if ( Sql.ToInteger(row["DELETED"]) == 0 )
				{
			...
			// 06/04/2008   Some customers have reported a problem with the indexes. 
			RenumberIndexes();
		}

		public void DynamicTableInsert(int nFieldIndex)
		{
			// 01/08/2006   Insert just makes space by shifting the indexes up. 
			DataView vwFields = new DataView(dtFields);
			vwFields.RowFilter = "DELETED = 0"     ;
			vwFields.Sort      = LayoutIndexName() + " desc";
			foreach(DataRowView row in vwFields)
		...
		{
			//01/07/2006   Move up means to decrement. 
		...
		{
			//01/07/2006   Move down means to increment. 
		...
		// 06/04/2008   We need a way to fix indexes.
		public void RenumberIndexes()
		{
			int nFieldIndex = 0;
			// 06/10/2009   For the GridViews, be careful not to change the order of the first item as it typically will skip the checkbox. 
		...
		protected virtual void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbTransaction trn = Sql.BeginTransaction(con) )
						{
							try
							{
								...
								// 01/20/2010   We cannot validate the GRIDVIEW as duplicate fields are allowed. 
								...
								foreach(DataRowView row in vwFields)
								{
									if ( row.Row.RowState == DataRowState.Modified || row.Row.RowState == DataRowState.Added )
									{
										...
										foreach(IDbDataParameter par in cmdUpdate.Parameters)
										{
											...
											// 02/14/2013   When copying a field, we need to skip the ID so that a new record will be created. 
											...
											{
												// 01/09/2006   Make sure to use ToDBString to convert empty stings to NULL. 
												switch ( par.DbType )
												{
								...
								// 02/14/2013   No need to delete records when copying a layout. 
								if ( !ctlLayoutButtons.CopyLayout.Visible )
								{
									...
									if ( !Sql.IsEmptyString(sLAYOUT_NAME_FIELD) )
									{
										...
										// 08/02/2010   Exclude the JavaScript field in the count. 
								...
								trn.Commit();
								// 01/09/2006   Make sure to clear the cache so that the changes will take effect immediately. 
								ClearCache(sVIEW_NAME);
								// 02/14/2013   Allow the layout tree to be rebuilt. 
								if ( ctlLayoutButtons.CopyLayout.Visible )
									ctlSearch.Bind();
							}
							catch(Exception ex)
							...
							// 01/04/2009   Move the redirect out of the try/catch so that the thread-abort does not generate a false error. 
				...
				// 02/13/2013   Provide a way to copy the list. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				// 01/08/2006   The viewstate is no longer disabled, so we can go back to using ctlSearch.NAME.
				string sNAME = ctlSearch.NAME;  //Sql.ToString(Request[ctlSearch.ListUniqueID]);
				// 07/27/2010   Keep the Search visible, especially now that we are showing a TreeView. 
				//ctlSearch     .Visible = Sql.IsEmptyString(sNAME);
				ctlLayoutButtons.Visible = !Sql.IsEmptyString(sNAME);
				// 05/22/2009   We need to pass the view name to the Export popup. 
				ctlLayoutButtons.VIEW_NAME = sNAME;
				// 09/08/2007   Add a list header so we will know what list we are working on. 
				...
				if ( ctlNewRecord == null )
				{
					// 07/27/2010   The NewRecord was moved from the parent to this control, so we don't need to find it. 
				...
				{
					// 01/08/2006   We are having a problem with the ViewState not loading properly.
					...
					SetPageTitle(sNAME);
					// 07/27/2010   Binding the page at this time is causing a TreeView exception. 
					//Page.DataBind();
					...
					LayoutView_Bind(false);
					// 07/27/2010   When a layout changes, we need to clear the NewRecord panel. 
					if ( ctlNewRecord != null )
						ctlNewRecord.Clear();
		...
		{    
			...
			if ( IsPostBack )
			{
				// 11/30/2006   We were having a problem with viewstate. Make sure to load the fields inside Page_Init. 
				LoadFieldState();

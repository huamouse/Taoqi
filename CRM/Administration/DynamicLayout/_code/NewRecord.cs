...
namespace Taoqi.Administration.DynamicLayout
{
	...
	public class NewRecord : SplendidControl
	{
		...
		public virtual void Clear()
		{
			...
			// 04/19/2009   When clearing the NewRecord control, we must also reset the data field listbox. 
			...
			// 07/27/2010   ClearSelection should be safer than SelectedIndex. 
			...
			this.Visible = false;
			// 05/17/2010   Make sure to update the visibility flags. 
		...
		public string DATA_LABEL
		{
			...
			set
			{
				// 01/10/2006   Always try and select the label from the list. 
				txtDATA_LABEL.Text = value;
				// 08/19/2010   Check the list before assigning the value. 
				// 09/13/2010   SetSelectedValue will not throw an exception.  If not found, manually check the free form flag. 
				...
				{
					// 01/10/2006   If value does not exist, then go to free form. 
					txtDATA_LABEL.Text = value;
		...
		protected void chkFREE_FORM_LABEL_CheckedChanged(Object sender, EventArgs e)
		{
			if ( !chkFREE_FORM_LABEL.Checked )
			{
				// 01/10/2006   Validate the ability to turn off free form.
				...
				{
					// 01/10/2006   If there is an error, then go back to free form. 
					chkFREE_FORM_LABEL.Checked = true;
				}
			}
			txtDATA_LABEL.Visible =  chkFREE_FORM_LABEL.Checked;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					if ( !this.IsPostBack || lstLIST_NAME.Items.Count == 0 )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								da.Fill(dt);
								// 01/07/2006   Having a problem with manual list inserts getting lost.  Try modifying the table. 
								// 07/15/2007   There are a bunch more cached lists that need to be added. 
								// 08/17/2007   We forgot to include the blank string.  
								// Without the blank string, all data fields would need to be associated with a list, which is a major problem. 
								// 11/19/2008   Use the CustomCaches list to prevent any future omissions.
								for ( int i = 0; i < SplendidCache.CustomCaches.Length; i++ )
								{
									DataRow row = dt.NewRow();
									row["LIST_NAME"] = SplendidCache.CustomCaches[i].Name;
									dt.Rows.InsertAt(row, i);
								}
								// 12/04/2008   Add the blank string back. 
								dt.Rows.InsertAt(dt.NewRow(), 0);
					...
					// 02/16/2010   Dynamically populate the module list. 
					if ( !this.IsPostBack || lstMODULE_TYPE.Items.Count == 0 )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								DataTable dt = new DataTable();
								da.Fill(dt);
								// 02/16/2010   Add the blank string back. 
								dt.Rows.InsertAt(dt.NewRow(), 0);
						...
						*/
						// 11/10/2011   vwMODULES_LayoutViews requires the Report Enabled flag.  Using ModulesPopup is a better solution. 
						DataTable dtModules = SplendidCache.ModulesPopups(this.Context).Copy();
						// 04/20/2012   Need to set HAS_POPUP to 1. 

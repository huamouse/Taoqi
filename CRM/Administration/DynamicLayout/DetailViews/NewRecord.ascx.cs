...
namespace Taoqi.Administration.DynamicLayout.DetailViews
{
	...
	public class NewRecord : DynamicLayout.NewRecord
	{
		...
		public override void lstFIELD_TYPE_Changed(Object sender, EventArgs e)
		{
			...
			{
				// 08/02/2010   Show the URL fields for a string so that we can add a LinkedIn icon. 
				...
				// 04/20/2012   Show the Module Type for a HyperLink. 
			...
			// 08/02/2010   Not sure why, but there are times when we set the Visible flag to true, but it rejects the setting and stays false. 
			//trDATA_FIELD.Visible = trDATA_LABEL.Visible;
			// 09/12/2009   Set default format. 
			...
			if ( lstFIELD_TYPE.SelectedValue == "IFrame" )
			{
				...
				// 06/16/2010   Set the default height to 200. 
				int nValue = 0;
			...
			else
			{
				...
				// 08/02/2010   Don't clear the target.  It is also being used by JavaScript. 
				//txtURL_TARGET.Text = "";
			}
		}

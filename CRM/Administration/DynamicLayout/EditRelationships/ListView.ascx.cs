...
namespace Taoqi.Administration.DynamicLayout.EditRelationships
{
	/// <summary>
	...
	public class ListView : SplendidControl
	{
		...
		protected void grdMain_ItemCreated(object sender, DataGridItemEventArgs e)
		{
			...
			else if ( e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem )
			{
				...
				if ( row != null )
				{
					if ( !(Sql.ToBoolean(row["RELATIONSHIP_ENABLED"]) && (Taoqi.Security.AdminUserAccess(m_sMODULE, "edit") >= 0)) )
						e.Item.CssClass += " nodrag nodrop";
				}
			}
		}
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				else if ( e.CommandName == "Relationships.MoveDown" )
				{
					if ( Sql.IsEmptyGuid(gID) )
						throw(new Exception("Unspecified argument"));
					// 09/08/2007   The name is not MoveDown because Oracle will truncate to 30 characters
				...
				// 01/04/2005   If the list changes, reset the cached values. 
				// 12/11/2008   We needed to clear the view name.
				SplendidCache.ClearEditViewRelationships(ctlSearch.NAME);
				EDITVIEWS_RELATIONSHIPS_BindData(true);
			}
			catch(Exception ex)
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// 07/26/2010   Lets stop hiding the search after selection. 

...
namespace Taoqi.Administration.DynamicLayout.EditRelationships
{
	...
	public class SearchBasic : SearchControl
	{
		...
		{
			// 01/06/2006   Try disabling viewstate of DetailView to prevent viewstate error. 
			if ( !this.IsPostBack || !Parent.EnableViewState )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					// 04/19/2010   The EditView Relationships will only apply to EditViews. 
						...
						try
						{
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								using ( DataTable dt = new DataTable() )
								{
									...
									// 01/08/2006   The viewstate is no longer disabled, so this is not necessary. 
									/*
									try
									{
										// 01/06/2006   If viewstate has been disabled, then recall the submitted value. 

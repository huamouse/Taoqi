				...
				if ( Page.IsValid )
				{
					...
					if ( Page.IsValid )
					{
						...
						try
						{
							...
							// 01/16/2006   Update language cache. 
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											// 01/20/2006   Don't allow the name to be changed.  Require a new term. 
											...
											// 08/19/2010   Check the list before assigning the value. 
											// 08/31/2010   SetSelectedValue will return false and not throw an exception if not found. 
											if ( !Utils.SetSelectedValue(lstMODULE_NAME, sMODULE_NAME) )
											{
												// 01/12/2006   If module does not exist in our table, then add it to prevent data loss. 

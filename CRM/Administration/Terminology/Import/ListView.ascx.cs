...
namespace Taoqi.Administration.Terminology.Import
{
	...
	public class ListView : SplendidControl
	{
		...
		// 10/05/2009   We need to add the command handler to the language pack. 
		...
		protected void ImportFromStream(Stream stm)
		{
			...
			using ( ZipInputStream stmZip = new ZipInputStream(stm) )
			{
				...
				while ( (theEntry = stmZip.GetNextEntry()) != null )
				{
					...
					if ( sFileName != String.Empty )
					{
						...
						if ( theEntry.Name.EndsWith(".lang.php") )
						{
							...
							// 11/13/2006   SugarCRM still has not fixed their German language pack. Convert ge-GE to de-DE.
							if ( String.Compare(sLang, "ge-GE", true) == 0 )
								sLang = "de-DE";
							// 12/23/2008   Vietnamese needs to fix the SugarCRM international code. 
							else if ( String.Compare(sLang, "vn-VN", true) == 0 )
								sLang = "vi-VN";
							// 08/22/2007   Only insert the language record once. 
							if ( !hashLanguages.ContainsKey(sLang) )
							{
								...
								// 12/22/2008   Enable after inserting, just in case the language already exists and is currently disabled. 
							...
							{
								// 01/30/2008   Accumulate the errors. 
								sbErrors.AppendLine(theEntry.Name + ": " + ex.Message);
							}
						}
					}
				}
				// 01/12/2006   Update internal cache. 
				...
				// 01/13/2006   Clear the language cache. 
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmdLANGUAGES_InsertOnly = SqlProcs.cmdLANGUAGES_InsertOnly(con) )
				{
					...
					foreach ( XmlNode xTerm in nlLANGUAGES )
					{
						...
						{
							// 01/24/2014   The tag name is NAME not LANG. Also fix value. 
							if ( node.Name == "NAME" )
								sNAME = node.InnerText;
							// 10/05/2009   The correct field is InnerText.  This is because the nodes are elements not attributes. 
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "Next" )
			{
				...
				if ( Page.IsValid )
				{
					...
					try
					{
						// 07/03/2007   Increase timeout to support slower machines. 
						...
						if ( pstIMPORT != null )
						{
							if ( pstIMPORT.FileName.Length > 0 )
							{
								...
								// 10/05/2009   Allow direct import of XML file. 
						...
						using ( HttpWebResponse objResponse = (HttpWebResponse) objRequest.GetResponse() )
						{
							if ( objResponse != null )
							{
								...
								else
								{
									...
									// 10/05/2009   The production servers return "text/xml; charset=utf-8". 

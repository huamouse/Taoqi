...
namespace Taoqi.Administration.Terminology.Import
{
	/// <summary>
			...
			try
			{
				...
				if ( !Sql.ToBoolean(Request["PrecompileOnly"]) )
				{
					...
					if ( dt == null )
					{
						...
						try
						{
							// 10/05/2009   We need to be able to debug the production language packs. 
#if FALSE
...
#else
							// 11/30/2008   Change name of service level to Community. 
							// 07/11/2011   xml.Load() is not working. 
							...
							// 07/11/2011   The problem was a NULL UserAgent in SplendidInit.InitSession().  Keep these changes just in case there is a problem in the future. 
							...
							{
								string sResponse = HttpGetRequest("http://community.Taoqi.com/Administration/Terminology/Export/Languages.aspx");
							...
							{
								string sResponse = HttpGetRequest("http://enterprise.Taoqi.com/Administration/Terminology/Export/Languages.aspx");
								if ( !sResponse.StartsWith("<?xml") && sResponse.Contains("<?xml") )
									sResponse = sResponse.Substring(sResponse.IndexOf("<?xml"));
								xml.LoadXml(sResponse);
							}
							...
							{
								string sResponse = HttpGetRequest("http://professional.Taoqi.com/Administration/Terminology/Export/Languages.aspx");
								if ( !sResponse.StartsWith("<?xml") && sResponse.Contains("<?xml") )
									sResponse = sResponse.Substring(sResponse.IndexOf("<?xml"));
								xml.LoadXml(sResponse);
							}
						...
						{
							// 10/21/2009   We are getting regular errors on the first attempt, so lets always retry. 
						...
						// 07/20/2010   If we fail to get the document, then don't attempt to load it. 
						if ( xml.DocumentElement != null )
							dt = XmlUtil.CreateDataTable(xml.DocumentElement, "LanguagePack", new string[] {"Name", "Date", "Description", "URL"});
#if !DEBUG
						// 12/13/2008   Don't cache if there is no data. 

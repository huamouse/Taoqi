...
namespace Taoqi.Users
{
	...
	/// </summary>
	public class Logins : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwUSERS_LOGINS" + ControlChars.CrLf
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += grdMain.OrderByClause("LOGIN_DATE", "desc");

					if ( bDebug )
						RegisterClientScriptBlock("vwUSERS_LOGINS", Sql.ClientScriptBlock(cmd));
		...
		{
			this.Visible = (Taoqi.Security.AdminUserAccess(m_sMODULE, "view") >= 0);
			if ( !this.Visible )
			{
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("LOGIN_DATE");
			arrSelectFields.Add("IS_ADMIN"  );
			this.AppendGridColumns(grdMain, "Users.Logins", arrSelectFields);

            //2015.2.9
            this.grdMain.DataBinding += grdMain_DataBinding;
		}

        void grdMain_DataBinding(object sender, EventArgs e)
        {
            if (this.vwMain != null)
            {
             
                foreach (DataGridColumn col in this.grdMain.Columns)
                {

                    if (col.HeaderText == "注销时间" ||
                        col.HeaderText == "登录类型" ||
                        col.HeaderText == "远程主机" 
                        )
                    {

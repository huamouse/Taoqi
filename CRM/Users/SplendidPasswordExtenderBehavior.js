...
(function()
{
	var scriptName = 'ExtendedSplendidPassword';

	function execute()
	{
		Type.registerNamespace('Sys.Extended.UI');

		Sys.Extended.UI.SplendidPasswordExtenderBehavior = function(element)
		{
			Sys.Extended.UI.SplendidPasswordExtenderBehavior.initializeBase(this, [element]);
			...
			this._helpHandlePosition       =  Sys.Extended.UI.DisplayPosition.AboveSide;
			this._helpText                 = '';
			...
			this._complexityNumber  = 1;
			
			// 02/19/2011   Messages are now external. 
			this._msgRemainingCharacters  = '';
			this._msgRemainingNumbers     = '';
			this._msgRemainingLowerCase   = '';
			this._msgRemainingUpperCase   = '';
			this._msgRemainingMixedCase   = '';
			this._msgRemainingSymbols     = '';
			this._msgSatisfied            = '';
		}

		Sys.Extended.UI.SplendidPasswordExtenderBehavior.prototype =
		{
			initialize : function()
			{
				Sys.Extended.UI.SplendidPasswordExtenderBehavior.callBaseMethod(this, 'initialize');
				...
				{
					// 03/05/2011   We are going to allow a zero length password. 
					this._preferredPasswordLength = 0;  // Set to at least 10 chars as a preferred pwd length, even though this is very small.
				...
				// Get our initial password strength calculated values. This performs an initial calculation and sets up the helptext.
				// 03/05/2011   Lets save the help text on initialize and restore it.  This will allow us to use the lblError field as the postback error field. 
				var sInitialHelpText = '';
				if ( this._helpStatusLabelID )
				{
					var label = $get(this._helpStatusLabelID);
					if ( label )
					{
						sInitialHelpText = label.innerHTML;
					}
				}
				// 03/05/2011   The get password function will update the help text, so just restore the previous value after the call. 
				this._getSplendidPassword();
				if ( sInitialHelpText != '' )
				{
					var label = $get(this._helpStatusLabelID);
					if ( label )
					{
						label.innerHTML = sInitialHelpText;
					}
				}
			},
			...
			_createTextDisplayElement : function()
			{
				...
				{
					p.id = this.get_element().id + '_SplendidPassword';
				}
			...
			_setTextDisplayStyle : function(index)
			{
				...
				else
				{
					...
					if ( this._txtPwdStrengthCssClass && Sys.UI.DomElement.containsCssClass(this._displayDiv,this._txtPwdStrengthCssClass) )
					{
			...
			_setBarDisplayStyle : function(index)
			{
				...
				{
					if (this._barIndicatorCssClass && Sys.UI.DomElement.containsCssClass(this._barInnerDiv,this._barIndicatorCssClass))
					{
			...
			_createBarIndicatorDisplayElement : function()
			{
				...
				if (this.get_element().id)
				{
					d1.id = this.get_element().id + '_SplendidPasswordBar1';
					d2.id = this.get_element().id + '_SplendidPasswordBar2';
				}
			...
			_showStrength : function()
			{
				...
				var pwdStrength = this._getSplendidPassword();
				if (this._strengthIndicator == Sys.Extended.UI.StrengthIndicatorTypes.BarIndicator)
				{
			...
			_getSplendidPassword : function()
			{
				...
				// 02/19/2011   Trim leading and trailing spaces. 
				...
				{
					this._calcWeightings = '50;15;15;20';
					weights = this._calcWeightings.split(';');
				...
				if ( pwd.length < this._preferredPasswordLength )
					pwdRequirements = String.format(this._msgRemainingCharacters, this._preferredPasswordLength - pwd.length);
				...
				if (this._minimumNumericCharacters > 0)
				{
					...
					else
					{
						if ( pwdRequirements != '' )
							pwdRequirements += ', ';
						pwdRequirements += String.format(this._msgRemainingNumbers, this._minimumNumericCharacters - numCount);
					}
				}
				else
				{
				...
				if ( this._minLowerCaseChars > 0 && this._minUpperCaseChars > 0 )
				{
					...
					{
						if ( this._minLowerCaseChars > 0 && numLower >= this._minLowerCaseChars )
						{
						...
						else
						{
							if ( pwdRequirements != '' )
								pwdRequirements += ', ';
							pwdRequirements += String.format(this._msgRemainingLowerCase, this._minLowerCaseChars - numLower);
						}
						if ( this._minUpperCaseChars > 0 && numUpper >= this._minUpperCaseChars )
						{
							percentTotal += _ratioCas;
							complexity++;
						}
						else
						{
							if ( pwdRequirements != '' )
								pwdRequirements += ', ';
							pwdRequirements += String.format(this._msgRemainingUpperCase, this._minUpperCaseChars - numUpper);
						}
					}
					else
					{
						if ( pwdRequirements != '' )
							pwdRequirements += ', ';
						pwdRequirements += String.format(this._msgRemainingMixedCase, this._minLowerCaseChars + this._minUpperCaseChars);
					}
				}
				else if ( this._minLowerCaseChars > 0 || this._minUpperCaseChars > 0 )
				{
					var mixedcaseRegex = new RegExp('[a-z,A-Z]', 'g');
					var numMixed = this._getRegexCount(mixedcaseRegex, pwd);
					if ( numMixed >= (this._minLowerCaseChars + this._minUpperCaseChars) )
					{
						percentTotal += _ratioCas;
						complexity++;
					}
				...
				if ( this._minimumSymbolCharacters > 0 )
				{
					...
					if ( this._symbolCharacters != null && this._symbolCharacters != '' )
					{
						var _escapedSymbolCharacters = this._symbolCharacters.replace(/([\\\^\$*+[\]?{}.=!:(|)])/g, '\\$1');
					...
					if ( numCount < this._minimumSymbolCharacters )
					{
						if ( pwdRequirements != '' )
							pwdRequirements += ', ';
						pwdRequirements += String.format(this._msgRemainingSymbols, this._minimumSymbolCharacters - numCount);
					}
				}
				else
				{
				...
				// 02/20/2011   If the password meets the complexity requiements, then ignore the other failures. 
				if ( pwd.length >= this._preferredPasswordLength && complexity >= this._complexityNumber )
					pwdRequirements = '';
			...
			dispose : function()
			{
				...
				Sys.Extended.UI.SplendidPasswordExtenderBehavior.callBaseMethod(this, 'dispose');
			},
			...
			get_SymbolCharacters : function()
			{
				return this._symbolCharacters;
			},

			set_SymbolCharacters : function(value)
			{
				this._symbolCharacters = value;
			},

			get_ComplexityNumber : function()
			{
				return this._complexityNumber;
			},

			set_ComplexityNumber : function(value)
			{
				this._complexityNumber = value;
			},

			// 02/19/2011   Messages are now external. 
			get_MessageRemainingCharacters : function()
			{
				return this._msgRemainingCharacters;
			},

			set_MessageRemainingCharacters : function(value)
			{
				this._msgRemainingCharacters = value;
			},

			get_MessageRemainingNumbers : function()
			{
				return this._msgRemainingNumbers;
			},

			set_MessageRemainingNumbers : function(value)
			{
				this._msgRemainingNumbers = value;
			},

			get_MessageRemainingLowerCase : function()
			{
				return this._msgRemainingLowerCase;
			},

			set_MessageRemainingLowerCase : function(value)
			{
				this._msgRemainingLowerCase = value;
			},

			get_MessageRemainingUpperCase : function()
			{
				return this._msgRemainingUpperCase;
			},

			set_MessageRemainingUpperCase : function(value)
			{
				this._msgRemainingUpperCase = value;
			},

			get_MessageRemainingMixedCase : function()
			{
				return this._msgRemainingMixedCase;
			},

			set_MessageRemainingMixedCase : function(value)
			{
				this._msgRemainingMixedCase = value;
			},

			get_MessageRemainingSymbols : function()
			{
				return this._msgRemainingSymbols;
			},

			set_MessageRemainingSymbols : function(value)
			{
				this._msgRemainingSymbols = value;
			},

			get_MessageSatisfied : function()
			{
				return this._msgSatisfied;
			},

			set_MessageSatisfied : function(value)
			{
				this._msgSatisfied = value;
			}
		}


		Sys.Extended.UI.SplendidPasswordExtenderBehavior.registerClass('Sys.Extended.UI.SplendidPasswordExtenderBehavior', Sys.Extended.UI.BehaviorBase);
		Sys.registerComponent(Sys.Extended.UI.SplendidPasswordExtenderBehavior, { name: 'SplendidPassword' });

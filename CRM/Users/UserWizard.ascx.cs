...
namespace Taoqi.Users
{
	...
	public class UserWizard : SplendidControl
	{
			...
			if ( LAST_NAME_REQUIRED.IsValid && EMAIL1_REQUIRED.IsValid && EMAIL1_VALIDATOR.IsValid )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					SqlProcs.spUSERS_Update
						...
						// 03/25/2011   Create a separate field for the Facebook ID. 
                        ...
                        //, Sql.ToString (rowCurrent["C_Company"           ])
                        , Sql.ToBoolean(rowCurrent["C_IsReceivePaymentEmail"])
                       , Sql.ToInteger(rowCurrent["C_Sex"])
						);
				}
			}
		}
		...
		protected void SaveMailSettings()
		{
			...
			// 07/09/2010   SMTP values belong in the OUTBOUND_EMAILS table. 
			SqlProcs.spOUTBOUND_EMAILS_UpdateUser(Security.USER_ID, MAIL_SMTPUSER.Text, sENCRYPTED_EMAIL_PASSWORD);
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					// 07/29/2010   The Mail Settings is the last page, so we will get a Finish event and not a Next event. 
				...
				else if ( e.CommandName == "Smtp.Test" )
				{
					...
					// 07/29/2010   Only save the password if the send was successful. 
					if ( !Sql.IsEmptyString(sMAIL_SMTPPASS) )
					{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				try
				{
					...
					// 07/09/2010   Need to initialize the date/time dropdowns. 
					// 07/09/2010   Need to initialize all locale values from the system defaults. 
					try
					{

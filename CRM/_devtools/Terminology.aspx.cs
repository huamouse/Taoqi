	...
	public class Terminology : System.Web.UI.Page
	{
		protected void DumpTerminology(StringBuilder sb, IDbCommand cmd, string sProcedureName, int nNAME_MaxLength)
		{
			using ( SqlDataReader rdr = (SqlDataReader) cmd.ExecuteReader() )
			{
				...
				while ( rdr.Read() )
				{
					// 04/29/2006   DB2 is having a heap error SQL0954C.  
					...
					if ( nCount > 250 )
					{
						// 01/27/2007   We have had one report of a Splendid Configuration Wizard timeout,
					...
					for ( int nColumn=0 ; nColumn < rdr.FieldCount ; nColumn++ )
					{
						...
						// 05/19/2008   Unicode strings must be marked as such, otherwise unicode will go in as ???.
						...
						// 11/21/2005   Align the name field. 
						if ( nColumn == 0 )
						{
							// 08/20/2010   NAME will be null in some lists, such as published_reports_dom and saved_reports_dom. 
		...
		protected string DumpAllTerms(IDbConnection con, string sLANG)
		{
			...
			else
			{
				...
				sb.AppendLine("call dbo.spSqlDropProcedure('spTERMINOLOGY_Defaults_" + sLANG.Replace("-", "_") + "')");
				sb.AppendLine("/");
				sb.AppendLine("");
				sb.AppendLine("Create Procedure dbo.spTERMINOLOGY_Defaults_" + sLANG.Replace("-", "_") + "()");
				...
				sb.AppendLine("print 'TERMINOLOGY " + sLANG + "';");
				sb.AppendLine("GO");
				sb.AppendLine();
			...
			using ( IDbCommand cmd = con.CreateCommand() )
			{
				...
				// 03/06/2006   Oracle is case sensitive, and we modify the case of L10n.NAME to be lower. 
				Sql.AddParameter(cmd, "@LANG", sLANG.ToLower());
				DumpTerminology(sb, cmd, "spTERMINOLOGY_InsertOnly", nNAME_MaxLength);
				sb.AppendLine();
				...
				else
				{
					...
					sb.AppendLine("call dbo.spTERMINOLOGY_Defaults_" + sLANG.Replace("-", "_") + "()");
					sb.AppendLine("/");
					sb.AppendLine("");
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// 05/24/2008   Allow to be run remotely. 
			...
			try
			{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								sSQL = "select *          " + ControlChars.CrLf
								     + "     , (select count(*) from vwTERMINOLOGY where LANG = vwLANGUAGES.NAME) as TERM_COUNT" + ControlChars.CrLf
								     + "  from vwLANGUAGES" + ControlChars.CrLf
								     + " order by NAME    " + ControlChars.CrLf;
								cmd.CommandText = sSQL;
								using ( SqlDataReader rdr = (SqlDataReader) cmd.ExecuteReader() )
								{
									Response.Write("<html><body><h1>Terminology</h1>" + ControlChars.CrLf);
									...
									while ( rdr.Read() )
									{
										...
										Response.Write("<td><a href=\"Terminology.aspx?Lang=" + Sql.ToString(rdr["NAME"]) + "\">Export</a></td>" + ControlChars.CrLf);
										if ( Sql.ToString(rdr["NAME"]) != "en-US" )
											Response.Write("<td><a href=\"Translate.aspx?Lang="   + Sql.ToString(rdr["NAME"]) + "\">Translate</a></td>" + ControlChars.CrLf);
										Response.Write("</tr>" + ControlChars.CrLf);
									}
				...
				else
				{
					...
					if ( String.Compare(sLANG, "all", true) != 0 )
					{
						...
						// 02/16/2010   Must include all parts of the name in the encoding. 
						Response.AddHeader("Content-Disposition", "attachment;filename=" + Utils.ContentDispositionEncode(Request.Browser, "TERMINOLOGY " + sLANG + ".2.sql"));
					}
						...
						if ( String.Compare(sLANG, "all", true) == 0 )
						{
							...
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									{
										da.Fill(dt);
										if ( !Directory.Exists(Server.MapPath("~/Terminology.All")) )
											Directory.CreateDirectory(Server.MapPath("~/Terminology.All"));
										foreach ( DataRow row in dt.Rows )
										{
											...
											try
											{
												string sTerminologyPath = Server.MapPath("~/Terminology.All/TERMINOLOGY " + sLANG + ".2.sql");

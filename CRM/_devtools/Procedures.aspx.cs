...
namespace Taoqi._devtools
{
	...
	public class Procedures : System.Web.UI.Page
	{
		...
		// 03/06/2010   Make the BuildWrapper function public and static so that it can be reused in the ModuleBuilder. 
		...
		{
			// 10/07/2009   We need to prevent the use of the system transaction function as a stand-alone call.  It makes no sense. 
			...
			else
			{
				...
				for ( int j = 0; j < colRows.Count; j++ )
				{
					...
					// 06/23/2005   Modified User ID is automatic. 
				...
				if ( bTransaction )
				{
					...
					{
						// 11/19/2006   Skip first parameter if MODIFIED_USER_ID. 
			...
			if ( !bCreateCommand )
			{
				...
				// 05/01/2006   All commands now use a transaction.  This is because Oracle does not have a transaction hierarchy. 
				...
				sb.AppendLine(TabSpace(nIndent) + "{");
				nIndent++;
				// 05/01/2006   All commands now use a transaction. 
				...
				// 08/14/2005   Truncate procedure names on a case-by-case basis. 
			...
			for ( int j = 0 ; j < colRows.Count; j++ )
			{
				...
				*/
				...
				if ( !bCreateCommand )
				{
					...
					// http://support.microsoft.com/kb/827366/
					if ( sBareName == "MODIFIED_USER_ID" )
                        sb.AppendLine(TabSpace(nIndent) + "IDbDataParameter par" + sBareName + Strings.Space(nSpace-1) + "= Sql.AddParameter(cmd, \"" + sName + "\"" + Strings.Space(nSpace-2) + ", " + " Security.AccountID" + Strings.Space(nSpaceCsPrefix-2) + ");");
					else if ( sSqlDbType == "SqlDbType.NVarChar" )
					{
						// 09/15/2009   For nvarchar(max), don't specify a length. 
						if ( nMaxLength == -1 )
							sb.AppendLine(TabSpace(nIndent) + "IDbDataParameter par" + sBareName + Strings.Space(nSpace-1) + "= Sql.AddParameter(cmd, \"" + sName + "\"" + Strings.Space(nSpace-2) + ", " + sCsPrefix + sBareName + Strings.Space(nSpaceCsPrefix-2) + ");");
						else
					...
					{
						// 09/15/2009   For varchar(max), don't specify a length. 
						if ( nMaxLength == -1 )
			...
			if ( !bCreateCommand )
			{
				...
				// 03/07/2010   Move the Trace function to the Sql class. 
				// 02/10/2012   WORKFLOW and SYSTEM procedures are plentyful and not useful in the trace. 
				...
				for ( int j = 0 ; j < colRows.Count; j++ )
				{
					...
					if ( bIsOutput )
					{
						...
						int nSpaceCsPrefix = 3 + nColumnAlignmentSize - sBareName.Length - sCsPrefix.Length;
						nSpaceCsPrefix = Math.Max(2, nSpaceCsPrefix);
						// 04/25/2008   ansistring needs to be treated like a string. 
						sCsType = (sCsType == "ansistring") ? "string" : sCsType;
						switch ( sCsType )
						{
							case "string":
			...
			{
				// 02/20/2006   Need to set the direction. 
				for ( int j = 0 ; j < colRows.Count; j++ )
				{
					DataRow row = colRows[j];
					string sName      = Sql.ToString (row["ColumnName"]);
			...
			if ( !bCreateCommand )
			{
				if ( !bTransaction )
				{
					...
					// 12/25/2008   Re-throw the original exception so as to retain the call stack. 
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
            ...
            if (!Taoqi.Security.isAdmin || Request.ServerVariables["SERVER_NAME"] != "localhost")
                return;
			StringBuilder sb = new StringBuilder();
			sb.AppendLine("/**********************************************************************************************************************");
			sb.AppendLine(" * Taoqi is a Customer Relationship Management program created by Taoqi Software, Inc. ");
			sb.AppendLine(" * Copyright (C) 2005-2011 Taoqi Software, Inc. All rights reserved.");
			sb.AppendLine(" * ");
			...
			sb.AppendLine(" * ");
			sb.AppendLine(" * You can contact Taoqi Software, Inc. at email address support@Taoqi.com. ");
			sb.AppendLine(" * ");
			...
			// 04/13/2006   Use existing connection to generate procedures. 
			DbProviderFactory dbf = DbProviderFactories.GetFactory();
			using ( SqlConnection con = dbf.CreateConnection() as SqlConnection )
			{
				string sSQL;
				con.Open();
				sb.AppendLine("namespace Taoqi");
				sb.AppendLine("{");
				...
				sb.AppendLine("			System.Diagnostics.Trace.WriteLine(\"SqlProcs.Trace:	exec dbo.\" + Sql.ExpandParameters(cmd) + \";\");");
				sb.AppendLine("#endif");
				sb.AppendLine("		}");
				sb.AppendLine();
				...
				sb.AppendLine("				default:  throw(new Exception(\"Unknown stored procedure \" + sProcedureName));");
				sb.AppendLine("			}");
				sb.AppendLine("			// 11/11/2008   PostgreSQL has issues treating integers as booleans and booleans as integers. ");
			...
			try
			{
				// 01/16/2008   Procedures.aspx was moved to the _devtools folder, 

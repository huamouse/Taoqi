    ...
    public class ListView : SplendidControl
    {
        ...
        protected void grdMain_OnSelectMethod(int nCurrentPageIndex, int nPageSize)
        {
            ...
            using (IDbConnection con = dbf.CreateConnection())
            {
                ...
                {
                    string sTABLE_NAME = "TQMarketInformation";
            ...
            try
            {
                ...
                using (IDbConnection con = dbf.CreateConnection())
                {
                    ...
                    using (IDbCommand cmd = con.CreateCommand())
                    {
                        ...
                        cmd.CommandText = "  from vwTQMarketInformation_List" + ControlChars.CrLf
                                        + " where 1 = 1           " + ControlChars.CrLf;
                        // 05/10/2007   ACL_ACCESS.OWNER is not defined for the EMPLOYEES module. 
                        //Security.Filter(cmd, m_sMODULE, "list");
                        //自定义搜索框功能
                        if (!Sql.IsEmptyString(this.txtSearch.Value))
                        {
                            cmd.CommandText += " and C_Title like '%" + this.txtSearch.Value + "%' ";
                        }
                        //ctlSearchView.SqlSearchClause(cmd);
        ...
        public void c_company_btn(string c_company)
        {
            /*
            grdMain.DataBind();
            DataTable dtHonourList = _code.CommonForCore.GetProcedureDataSet("spTQMarketInformations_RedTop10_Report", "@sector", c_company);
            ...
            DataTable dtBlackList = _code.CommonForCore.GetProcedureDataSet("spTQMarketInformations_BlackTop10_Report", "@sector", c_company);

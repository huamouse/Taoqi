	...
	public class Modules : System.Web.Services.WebService
	{
		...
		public bool AddToFavorites(string sMODULE, Guid gID)
		{
			...
			//try
			{
				...
				// 03/31/2012   Use the standard filter to verify that the user can view the record. 
				if ( !Sql.IsEmptyString(sMODULE) && !Sql.IsEmptyGuid(gID) && Taoqi.Security.GetUserAccess(sMODULE, "view") >= 0 )
				{
		...
		public bool RemoveFromFavorites(string sMODULE, Guid gID)
		{
			...
			//try
			{
				...
				// 03/31/2012   No need to validate on remove as the item would not be in the list if the user did not have access to it. 
				if ( !Sql.IsEmptyString(sMODULE) && !Sql.IsEmptyGuid(gID) && Taoqi.Security.GetUserAccess(sMODULE, "view") >= 0 )
				{

...
namespace Taoqi._controls
{
	...
	public class RelatedSelect : SplendidControl
	{
		...
		public DataTable LineItems
		{
			get
			{
				// 08/31/2009   When called from within the SearchView control, dtLineItems is not initialized. 
				if ( dtLineItems == null )
			...
			set
			{
				...
				// 08/31/2009   Add a blank row after loading. 
		...
		public string RELATED_SET_LIST
		{
			get
			{
				...
				if ( dtLineItems != null )
				{
					...
					{
						// 08/23/2009   Although the RELATED_ID should never be NULL or Empty, check for this condition anyway. 
			...
			if ( dt == null )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						// 09/19/2010   It would not be practical to navigate a large drop down. 
			...
			if ( e.Row.RowType == DataControlRowType.DataRow )
			{
				...
				if ( !bSupportsPopups )
				{
					...
					if ( lstRELATED_ID != null )
					{
						try
						{
							// 02/07/2010   Defensive programming, we don't need to convert the Eval result to a string before using it. 
		...
		protected void grdMain_RowDeleting(object sender, GridViewDeleteEventArgs e)
		{
			if ( dtLineItems != null )
			{
				...
				// 08/11/2007   Allow an item to be manually added.  Require either a product ID or a name. 
		...
		protected void grdMain_RowUpdating(object sender, GridViewUpdateEventArgs e)
		{
			if ( dtLineItems != null )
			{
				...
				if ( gRELATED_ID != Guid.Empty )
				{
					...
					// 12/07/2007   aCurrentRows is defined above. 
					//DataRow[] aCurrentRows = dtLineItems.Select(String.Empty, String.Empty, DataViewRowState.CurrentRows);
				...
				// 02/07/2010   Defensive programming, check for valid spnAjaxErrors control. 
		...
		public void LoadLineItems(Guid gPARENT_ID)
		{
			...
			if ( !bIsPostBack )
			{
				...
				{
					// 08/24/2009   We need to create another connection, even though there is usually an existing open connection. 
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								ViewState["LineItems"] = dtLineItems;
								grdMain.DataSource = dtLineItems;
								// 03/27/2007   Start with last line enabled for editing. 
								grdMain.EditIndex = dtLineItems.Rows.Count - 1;
								grdMain.DataBind();
							}
						}

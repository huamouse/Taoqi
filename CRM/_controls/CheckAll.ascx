<%@ Control Language="c#" AutoEventWireup="false" Codebehind="CheckAll.ascx.cs" Inherits="Taoqi._controls.CheckAll" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
<script runat="server">
/**********************************************************************************************************************
...
var sSelectedLabelFormat = '选择数：{0}';
...
// 11/27/2010   Special functions to add and remove checkbox values from a hidden field. 
function SplendidGrid_ToggleCheckbox(chk)
{
	...
	if ( fldSelectedItemsClientID != null )
	{
		...
		{
			// 09/20/2013   New method of removing an item from a comma-separated string. 
		...
		if ( fldSelectedLabelClientID != null )
		{
			// 09/20/2013   Need a new method for counting items that is not guid-specific. 
			var nItems = 0;
			if ( chk.name == 'chkMain' )
...
<asp:HyperLink ID="lnkSelectPage" NavigateUrl='<%# "javascript:SplendidGrid_CheckAll(1);" %>' Text='全选' CssClass="listViewCheckLink" runat="server" />
<asp:Literal Text=" - " runat="server" />

<%-- 
<asp:LinkButton ID="btnSelectAll" Visible="<%# bShowSelectAll %>" Text='选择全部' CommandName="SelectAll" OnCommand="Page_Command" CssClass="listViewCheckLink" runat="server" />
<asp:Literal Text=" - " Visible="<%# bShowSelectAll %>" runat="server" />
--%>

<asp:HyperLink ID="lnkDeselectAll" NavigateUrl='<%# "javascript:SplendidGrid_CheckAll(0);" %>' Text='取消全选' CssClass="listViewCheckLink" runat="server" />
&nbsp;

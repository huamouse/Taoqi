<%@ Control Language="c#" AutoEventWireup="false" Codebehind="SearchView.ascx.cs" Inherits="Taoqi._controls.SearchView" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
<%@ Import Namespace="Taoqi._controls" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divSearchView">
	<script type="text/javascript">
		...
		{
			// 01/24/2008   On PocketPC and SmartPhone, getElementById is not available. 
			var divSearch;
		...
		{
			var sASSIGNED_USER_ID = '<%= new Taoqi.DynamicControl(this, "ASSIGNED_USER_ID").ClientID %>';
			var sUNASSIGNED_ONLY  = '<%= new Taoqi.DynamicControl(this, "UNASSIGNED_ONLY" ).ClientID %>';
	...
	<asp:Panel ID="pnlSearchPanel" style='<%# (!IsPopupSearch && IsMobile) ? "display: none" : "display: inline" %>' runat="server">
		<asp:Panel ID="pnlSearchTabs" Visible="<%# ShowSearchTabs && !IsMobile %>" runat="server">
			<ul class="tablist">
				<li><asp:HyperLink ID="lnkBasicSearch"     Text='查询' CssClass=""        Runat="server" /></li>
				<li><asp:HyperLink ID="lnkAdvancedSearch"  Text='<%# L10n.Term(".LNK_ADVANCED_SEARCH" ) %>' CssClass="current" Visible="false" Runat="server" /></li>
		...
		<asp:Table SkinID="tabSearchForm" runat="server">
			<asp:TableRow>
				<asp:TableCell>
					...
					<asp:Panel ID="pnlSearchButtons" CssClass="button-panel" Visible="<%# ShowSearchButtons %>" runat="server">
						<asp:Table ID="tblSearchButtons" Width="100%" CellPadding="0" CellSpacing="0" style="padding-top: 4px;" runat="server">
							<asp:TableRow>
								<asp:TableCell HorizontalAlign="Right">
									<asp:Button ID="btnSearch" CommandName="Search" OnCommand="Page_Command" CssClass="button" Text='查询' ToolTip='查询' AccessKey='<%# L10n.AccessKey(".LBL_SEARCH_BUTTON_KEY") %>' Runat="server" />&nbsp;
									<asp:Button Visible="false" ID="btnClear"  CommandName="Clear"  OnCommand="Page_Command" CssClass="button" Text='清除' ToolTip='清除' AccessKey='<%# L10n.AccessKey(".LBL_CLEAR_BUTTON_KEY" ) %>' Runat="server" />

									<span Visible="false" runat="server">

<%@ Control Language="c#" AutoEventWireup="false" Codebehind="DashletHeader.ascx.cs" Inherits="Taoqi._controls.DashletHeader" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
<script runat="server">
/**********************************************************************************************************************
			...
			<asp:Table  BorderWidth="0" CellPadding="0" CellSpacing="0" runat="server">
				<asp:TableRow>
					<asp:TableCell Wrap="false">
						...
						<asp:Literal Text="&nbsp;" Visible="<%# ShowCommandTitles %>" runat="server" />
						<asp:LinkButton  ID="bntRefresh" CommandName="Refresh" OnCommand="Page_Command" CssClass="chartToolsLink"          Text='<%# L10n.Term("Dashboard.LBL_REFRESH") %>' Visible="<%# ShowCommandTitles %>" Runat="server" />
					</asp:TableCell>
					<asp:TableCell Wrap="false" Visible="<%# ShowEdit %>">
						...
						<span onclick="toggleDisplay('<%= DivEditName %>'); return false;">
							...
							<asp:Literal Text="&nbsp;" Visible="<%# ShowCommandTitles %>" runat="server" />
							<asp:LinkButton  ID="bntEdit" CommandName="Edit" OnCommand="Page_Command" CssClass="chartToolsLink"          Text='<%# L10n.Term("Dashboard.LBL_EDIT"  ) %>' Visible="<%# ShowCommandTitles %>" Runat="server" />
						</span>
					</asp:TableCell>
					<asp:TableCell Wrap="false">
						&nbsp;
						<span onclick="return confirm('<%= L10n.TermJavaScript("Home.LBL_REMOVE_DASHLET_CONFIRM") %>')">
							...
							<asp:Literal Text="&nbsp;" Visible="<%# ShowCommandTitles %>" runat="server" />
							<asp:LinkButton  ID="bntRemove" CommandName="Remove" OnCommand="Page_Command" CssClass="chartToolsLink"          Text='<%# L10n.Term(".LBL_REMOVE") %>' Visible="<%# ShowCommandTitles %>" Runat="server" />
						</span>
					</asp:TableCell>
				</asp:TableRow>
			</asp:Table>

...
namespace Taoqi._controls
{
	...
	public class TimePicker : SplendidControl
	{
		...
		public DateTime Value
		{
			get
			{
				// 12/31/2007   Exit early if neither value was specified. 
				// If only one value is specified, then the other is assumed to be 0. 
				if ( lstHOUR.SelectedValue == "" && lstMINUTE.SelectedValue == "" )
					return DateTime.MinValue;
				// 12/31/2007   Use 1900 as the base year because that is the minimum for SQL Server.  .NET can go down to 00/00/0000. 
		...
		private void SetDate()
		{
			...
			if ( lstMINUTE.Items.Count == 0 )
			{
				...
				// 01/26/2008   Move the empty string to the top. 
			...
			{
				// 12/31/2007   For a new record, default to Nothing. 
				// 01/26/2008   Nothing was moved to the top. 

...
namespace Taoqi.TQOrderLogistics
{
    ...
    public class ListView : SplendidControl
    {
        ...
        private void Page_Load(object sender, System.EventArgs e)
        {
            ...
            this.Visible = (Taoqi.Security.GetUserAccess(m_sMODULE, "list") >= 0) && (Security.isEmployee == 1);
            if (!this.Visible)
                return;
            ...
            try
            {
                ...
                using (IDbConnection con = dbf.CreateConnection())
                {
                    ...
                    using (IDbCommand cmd = con.CreateCommand())
                    {
                        ...
                        if (!Security.isAdmin)
                        {
                            cmd.CommandText += " and BuyerID = @AccountID" + ControlChars.CrLf;

                            Sql.AddParameter(cmd, "@AccountID", Security.AccountID);
                        }
                        if (!Sql.IsEmptyString(this.txtKeyword.Text))
                        {
                            cmd.CommandText += "and C_PlateNumber like'%" + this.txtKeyword.Text + "%'or SN like '%" + this.txtKeyword.Text + "%'";
                        ...
                        {
                            cmd.CommandText = "select " + Sql.FormatSelectFields(arrSelectFields) + cmd.CommandText;

                            if (Page.IsPostBack)
                            {
                                string keywords = this.txtKeyword.Text.Trim();

                                if (keywords.Length > 0)
                                {

                                    cmd.CommandText += " and (C_PlateNumber like @C_PlateNumber or SN like @SN) " + ControlChars.CrLf;

                                    Sql.AddParameter(cmd, "@C_PlateNumber", "%" + keywords + "%");
                                    Sql.AddParameter(cmd, "@SN", "%"+keywords+"%");
                                }
                            }

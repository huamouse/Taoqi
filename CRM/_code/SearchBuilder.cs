...
namespace Taoqi
{
	...
	public class SearchBuilder
	{
		...
		private   bool     m_bFeatureSINGLE = false ;  //Single letter filtering.  01/17/2005   Don't do this. 
		...
		// 07/16/2006   We need to know the database platform in order to build the like escape clause properly. 
		public SearchBuilder(string str, IDbCommand cmd)
		{
			m_cmd = cmd;
			// 09/02/2009   No need to create a new connection when we already have a command object. 
		...
		protected string EscapeLike(string str)
		{
			...
			// 07/16/2006   SQL Server, Oracle and DB2 all support the ESCAPE clause. 
		...
		protected void ParseInput(string sInput)
		{
			...
			{
				// 09/02/2009   Now that we know the database platform, only use the upper clause if Oracle or DB2. 
		...
		public string BuildQuery(string sCondition, string sField, bool bTextField)
		{
			...
			sbSqlQuery.Append("(");
				...
				// 07/16/2006   Now that we know the database platform, only use the upper clause if Oracle or DB2. 
			// 09/02/2008   PostgreSQL is case significant. 
			if ( m_bIsOracle || m_bIsDB2 || m_bIsPostgreSQL )
			{
				//Can't do an upper() on NTEXT.  SQL Server complains. 
				// 01/16/2008   We don't use the bTextField anymore, and I could not find any places where we searched an NTEXT field anyway. 
				//if ( !bTextField )
					sField = "upper(" + sField + ")";
			}
			...
			foreach ( string sThisToken in m_arrTokens )
			{
				if      ( m_bFeatureMIMUS && sThisToken == "-" )
					...
					nNotFlag = 2;
				// 01/20/2010   AND, OR and NOT clauses have been broken for a very long time.  We need to allow mixed-case for these tokens. 
					...
					nNotFlag = 1;
				// 12/17/2007   Add support for <, <=, > and >=. 
				else if ( m_bFeatureLESS  && sThisToken == "<" )
					nLessFlag = 1;
				else if ( m_bFeatureLESS  && sThisToken == ">" )
					nLessFlag = -1;
				// 12/17/2007   Add support for ! and !=.  They are both the same. 
					...
					if ( sThisToken.Length > 0 )
					{
						...
						if ( !m_bFeatureSINGLE || nNotFlag > 0 || sThisToken.Length > 1 || !Char.IsLetterOrDigit(sThisToken[0]) )
						{
							...
							if ( sThisToken[0] != '\"' || sThisToken.Length > 1 )
							{
								...
								// 12/17/2007   Wildcards will have a higher priority over greater than or less than. 
								if ( m_bFeatureSTAR && sToken.IndexOf('*') >= 0 )
								{
									...
									sToken = sToken.Replace("*", "%");
									// 12/04/2008   SQL Server needs to know to use the national character set. 
								...
								else
								{
									if ( nLessFlag == 0 )
									{
										if ( bEqualFlag )
										{
											// 12/17/2007   Single quotes need to be manually escaped. 

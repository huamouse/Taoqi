...
namespace Taoqi
{
	...
	public class LanguagePackImport
	{
		...
		*/
		...
		{
			// 11/13/2006   There is a file the the SugarCRM German language pack that has 4 parts.  
		...
		{
			// 10/09/2005   I expected UTF-8 to always work, but it does not.  Only use UTF-8 for known languages. 
		...
		public static void InsertTerms(string sPathName, ZipInputStream stmZip, bool bForceUTF8)
		{
			...
			if ( bForceUTF8 )
				enc = Encoding.UTF8;
			// 01/12/2006   Don't use using as it is clsing the zip stream. 
			StreamReader sr = new StreamReader(stmZip, enc, true);
			{
				string sData = String.Empty;
				try
				{
					// 01/12/2006   Problem importing it_it_3.5.1.langpack.zip, modules/OptimisticLock/language/it_it.lang.php
					...
					switch ( nMode )
					{
						...
						case 1:  // Extract entry name. 
						{
							...
							if ( chPunctuation != '\'' && chPunctuation != '\"' )
							{
								...
								{
									// Ignore comment. 
									// 05/19/2006   lang-pt_br12-351b.zip ends the line with '\n'
						...
						case 2: // Search for display name or inner list
						{
							...
							{
								nListStart++;
								// 10/09/2005   Just in case the file has been truncated, 
							...
							if ( sData[nListStart] == '=' && sData[nListStart+1] == '>' )
							{
								...
								nListStart++;
								int nValueStart = nListStart;
								// 10/09/2005   Watch out for 'LBL_CURRENCY_SYMBOL' => '\\', where the punctuation end has a slash before it. 
								//  'NTC_LOGIN_MESSAGE' => 'Prego fai login all\\\'applicazione.',
								// 10/09/2005   Use the bLastCharExcaped flag as a simple way to keep track of escaped characters 
								...
								else
								{
									...
									// 10/09/2005   Need to remove escape characters. 
									if ( strDisplayName.IndexOf('\\', 0) >= 0 )
									{
										// 10/09/2005  We must escape twice because we have escapes within escapes. \\\' should become '.  

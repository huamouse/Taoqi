	...
	public class Excel2007Reader
	{
		...
		public static string GetCellValue(SharedStringTablePart stringTablePart, Stylesheet style, Cell cell, ref string sFormat)
		{
			...
			else if ( cell.StyleIndex != null )
			{
				...
				// 05/10/2011   Lets get the format for future reference. 
				...
				if ( cellXfs.NumberFormatId != null )
				{
					...
					// 09/29/2013   It seems the only way to detect a date format is if it contacts a date formatting character. 
					...
					// 09/30/2013   We need to do the date conversion outside the switch statement so that standard formats get converted. 
						...
						if ( part != null && part.Worksheet != null )
						{
							...
							foreach ( Row rowExcel in data.Elements<Row>() )
							{
								...
								for ( int j = 0, nField = 0; j < cells.Count; j++, nField++ )
								{
									...
									// 10/01/2013   nField + 1 is wrong for XLSX (but correct for an Excel Spreadsheet).  nCellIndex is zero based. 

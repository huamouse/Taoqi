...
namespace Taoqi
{
	...
	public class Utils
	{
		...
		{
			// 05/09/2008   There is the rare possibility that the IDs are NULL.  Such as on the login page. 
			if ( !Sql.IsEmptyString(sTextID) )
			{
				// 03/28/2007   Fix to support Firefox, which passes the event object in the first parameter. 
				StringBuilder sb = new StringBuilder();
				sb.AppendLine("<script type=\"text/javascript\">");
				// 06/21/2009   We are having issues with the controls not being found.  Report the error instead of throwing an exception. 
		...
		public static string RegisterEnterKeyPress(string sTextID, string sButtonID)
		{
			...
			if ( !Sql.IsEmptyString(sTextID) && !Sql.IsEmptyString(sButtonID) )
			{
				...
				sb.AppendLine(" }");
				sb.AppendLine("}");
// 01/13/2010   Don't need these alerts any more. 
#if false
				sb.AppendLine("else");
				sb.AppendLine("{");
		...
		public static string RegisterSetFocus(string sTextID)
		{
			StringBuilder sb = new StringBuilder();
			sb.AppendLine("<script type=\"text/javascript\">");
			// 09/07/2009   Prevent a runtime error by checking if the element exists before setting the focus. 
			sb.AppendLine("if ( document.getElementById('" + sTextID + "') == null )");
		...
		public static string ValidateIDs(string[] arrID, bool bQuoted)
		{
			...
			foreach(string sID in arrID)
			{
				...
				{
					// 05/02/2006   Provide a more descriptive error message by including the ID. 
		...
		public static string FilterByACL(string sMODULE_NAME, string sACCESS_TYPE, string[] arrID, string sTABLE_NAME)
		{
			...
			if ( nACLACCESS >= 0 && arrID.Length > 0 )
			{
				if ( nACLACCESS == ACL_ACCESS.OWNER )
				{
					...
					{
						// 09/26/2006   The connection needed to be opened. 
						con.Open();
						string sSQL;
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							// 10/16/2006   Fix execute to allow more than one row. 
		...
		{
			// 08/21/2005   This function is also called after a user clicks Duplicate.
			...
			if ( !Sql.IsEmptyGuid(gID) )
			{
				...
				if ( pParent != null )
				{
					// 02/08/2007   The control is in the master page. 
                    ...
                    /*
                    Themes.Atlantic.TabMenu ctlTabMenu = pParent.Master.FindControl("ctlTabMenu") as Themes.Atlantic.TabMenu;
					if ( ctlTabMenu != null )
					{
						ctlTabMenu.Refresh();
						return;
		...
		// 08/19/2010   Use ListControl to be more generic. 
		public static void SetValue(ListControl lst, string sValue)
		{
			for ( int i=0 ; i < lst.Items.Count; i++ )
			{
				...
				{
					// 08/19/2010   We are doing a case-insignificant search, but then trying to set the case-significant value. 
		...
		public static string ExpandException(Exception ex)
		{
			...
			do
			{
				...
				// 08/13/2007   Only add the line break if there is more data. 
		...
		// 11/06/2009   We need a common way to detect the offline client. 
		...
		{
			// 05/08/2007   Keep the original procedure call so that we will get a compiler error if something changes. 
					...
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
						{
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								// 05/08/2007   Allow this to run until it completes. 
		...
		// 12/12/2009   We are going to call inside a thread as it can take a minute to run. 
		public static void UpdateSemanticModel(object o)
		{
			...
			// 12/12/2009   The semantic model is user-specific. 
			SplendidCache.ClearSet("SEMANTIC_MODEL.");
		}

		public static void BuildAllAuditTables()
		{
			// 12/02/2009   Keep the original procedure call so that we will get a compiler error if something changes. 
			bool bIncreaseTimeout = true;
			if ( !bIncreaseTimeout )
			{
				// 12/10/2009   The Offline Client does not support auditing. 
					...
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
						{
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								// 12/02/2009   Allow this to run until it completes. 
								cmd.CommandTimeout = 0;
								cmd.ExecuteNonQuery();
							}
							trn.Commit();
		...
		public static DataTable CheckVersion(HttpApplicationState Application)
		{
			...
			// 12/18/2008   Check the version of the appropriate service level. 
			...
			if ( String.Compare(sServiceLevel, "Basic", true) == 0 || String.Compare(sServiceLevel, "Community", true) == 0 )
				sVersionXmlURL = "http://community.Taoqi.com/Administration/Versions.xml";
			else if ( String.Compare(sServiceLevel, "Enterprise", true) == 0 )
				sVersionXmlURL = "http://enterprise.Taoqi.com/Administration/Versions.xml";
			else // if ( String.Compare(sServiceLevel, "Professional", true) == 0 )
				sVersionXmlURL = "http://professional.Taoqi.com/Administration/Versions.xml";
		...
		public static string UsageInfo(HttpApplicationState Application)
		{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					con.Open();
					string sSQL;
					// 10/21/2008   Exclude employees from user count. 
					...
					*/
					// 01/14/2008   SQL Server 2000 cannot count unique Guids. 
					...
					// 01/14/2008   Put the OS Version and SQL Version at the end as they may get truncated. 
			...
			// 01/19/2008   The application path seems useful, but will usually be /Taoqi. 
			sb.Append("&AppPath=" + HttpUtility.UrlEncode(Sql.ToString(Application["ApplicationPath"     ])));
			// 01/19/2008   The second least is the .NET version because it will almost always be the current shipping version. 
			sb.Append("&System="  + HttpUtility.UrlEncode(System.Environment.Version.ToString()            ));
			// 01/19/2008   The least important piece of information is the SugarVersion.  
			// 06/29/2011   Sugar compatibility is no longer useful. 
			//sb.Append("&Sugar="   + HttpUtility.UrlEncode(Sql.ToString(Application["CONFIG.sugar_version"])));
			return sb.ToString();
		}

		// 06/27/2009   Add quick function to select a single item in a listbox. 
		...
		// 12/20/2009   UrlPathEncode converts a space to a %20 whereas UrlEncode converts a space to +. 
		...
		{
			// 01/27/2011   Don't use GetFileName as the name may contain reserved directory characters, but expect them to be removed in Utils.ContentDispositionEncode. 
			sURL = sURL.Replace('\\', '_');
			sURL = sURL.Replace(':' , '_');
			// 12/20/2009   Make sure that the URL is not null. 
		...
		public static string GenerateVCard(DataRow row)
		{
			...
			// 01/01/2011   The Leads module will not have a BIRTHDATE field. 
		...
		public static DateTime CalDAV_ParseDate(string sDate)
		{
			...
			// 02/12/2012   An All-Day event does not include the time in the date strings. 
		...
		// 12/27/2012   VCalendar is used by the email reminders. 
		public static string GenerateVCalendar(DataRow row, bool bIncludeAlarm)
		{
			...
			// 12/27/2012   Outlook 2010 requires the Publish method, otherwise it will rename the file "not supported calendar message.ics". 
			sbVCalendar.AppendLine("METHOD:PUBLISH");
			// 01/05/2012   Lets try not using the timezone data and instead store date in UTC format. 
			/*
			Taoqi.TimeZone oTimeZone = Context.Application["TIMEZONE.TZID." + this.TZID] as Taoqi.TimeZone;
			if ( oTimeZone == null )
				oTimeZone = Context.Application["TIMEZONE.TZID.America/New_York"] as Taoqi.TimeZone;
			if ( oTimeZone == null )
			{
				...
				// 01/02/2012   Need to figure out how to convert week to 08.  There does not seem to be a clear pattern in the zoneinfo-outlook data. 
				...
				// 01/02/2012   Need to figure out how to convert week to 01.  There does not seem to be a clear pattern in the zoneinfo-outlook data. 
			...
			*/
			...
			// 01/15/2012   Not sure why, but DTSTAMP is not UTC time as it is 4 hours from the LAST-MODIFIED. 
			...
			// 01/07/2012   The folding includes the line header. RFC 5545 Page 9. 
			sbVCalendar.AppendLine(CalDAV_FoldLines("SUMMARY:" + CalDAV_Escape(Sql.ToString(row["NAME"]))));
			if ( !Sql.IsEmptyString(row["DESCRIPTION"]) )
			{
				// 01/05/2012   The VCalendar spec requires that we split the line at 75 characters. 
			...
			// 01/05/2012   Lets try not using the timezone data and instead store date in UTC format.  RFC 5545 Page 34. 
		...
		// 07/27/2012   Match the normalization within database function fnNormalizePhone. 
		...
		// 08/30/2013   Move jQuery registration to Utils class. 
		public static void RegisterJQuery(Page Page, ScriptManager mgrAjax)
		{
			if ( mgrAjax != null )
			{
				// 04/27/2012   Need to add support for favorites as the icons are visible. 
				...
				// 09/02/2013   The jQuery UI stylesheet will now be manually embedded in the master pages. 
                ...
                ScriptReference scrlayer = new ScriptReference("~/Include/javascript/layer/layer.js");
                ScriptReference scrlayerExt = new ScriptReference("~/Include/javascript/layer/extend/layer.ext.js");
                //ScriptReference scrlayer = new ScriptReference("~/Include/javascript/layer/layer.js");
                ...
                if (!mgrAjax.Scripts.Contains(scrlayer)) mgrAjax.Scripts.Add(scrlayer);
                if (!mgrAjax.Scripts.Contains(scrlayerExt)) mgrAjax.Scripts.Add(scrlayerExt);

...
namespace Taoqi
{
	...
	public class SplendidDynamic
	{
		...
		private static ImageButton CreateLayoutImageButton(Guid gID, string sCommandName, int nFIELD_INDEX, string sAlternateText, string sImageUrl, CommandEventHandler Page_Command)
		{
			...
			// 01/07/2006   The problem with the ImageButton Delete event was that the dynamically rendered ID 
		...
		*/
		...
		// 04/09/2008   Use SkinID to define the image. 
		// 12/24/2008   We need access to this function for the merge module. 
		...
		public static void ApplyDetailViewRules(string sDETAIL_NAME, SplendidControl parent, string sXOML_FIELD_NAME, DataRow row)
		{
			try
			{
				...
				DataTable dtFields = SplendidCache.DetailViewRules(sDETAIL_NAME);
				if ( dtFields.Rows.Count > 0 )
				{
					string sXOML = Sql.ToString(dtFields.Rows[0][sXOML_FIELD_NAME]);
					if ( !Sql.IsEmptyString(sXOML) )
		...
		public static void ValidateEditViewFields(string sEDIT_NAME, Control parent)
		{
			...
			// 11/27/2006   Make sure to include the TEAM_ID field since it does not use the UI_REQUIRED field. 
			// 01/01/2008   Make sure to include the ASSIGNED_USER_ID field since it does not use the UI_REQUIRED field. 
			// 04/02/2008   Include validated fields.
			...
			foreach(DataRowView row in dvFields)
			{
				...
				if ( sDATA_FIELD == "TEAM_ID" || sDATA_FIELD == "TEAM_SET_NAME" )
				{
					...
					if ( !bEnableTeamManagement )
					{
						// 01/01/2008   If Team Management is disabled, then we must disable the requirement. 
						bUI_REQUIRED = false;
					}
					else
					{
						// 09/21/2009   The same conversion that happens in AppendEditViewFields must occur here. 
						// 08/28/2012   DATA_FORMAT for TEAM_ID is 1 when we want to force ModulePopup. 
				...
				if ( bUI_REQUIRED && !Sql.IsEmptyString(sDATA_FIELD) )
				{
					...
					{
						// 12/17/2007   We could use START and END as the date suffixes, but AFTER and BEFORE are not currently used in field names. 
					...
					else if ( String.Compare(sFIELD_TYPE, "DatePicker", true) == 0 )
					{
						...
						if ( ctlDate != null )
						{
							// 03/04/2006   Only visible controls are validated. 
							if ( ctlDate.Visible )
								ctlDate.Validate();
						}
					}
					...
					// 10/21/2009   Add support for KBTags. 
				...
				if ( bUI_VALIDATOR )
				{
					...
					if ( ctl != null )
					{
						// 04/02/2008   Only visible controls are validated. 
						if ( ctl.Visible )
						{
							BaseValidator req = parent.FindControl(sDATA_FIELD + "_VALIDATOR") as BaseValidator;
							if ( req != null )
							{
								// 04/02/2008   Enable validator before validating page. 
								// If we leave the validator control enabled, then it may block an alternate action, like Cancel. 
								req.Enabled = true;
								req.Validate();
							}
		...
		/*
		// 01/16/2006   If we disable the validator, it will hide it's error message. 
...
*/
		...
		private static void LoadFile(Guid gID, Stream stm, IDbTransaction trn)
		{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					foreach(DataRow row in dtCustomFields.Rows)
					{
						...
						if ( ctlPARENT.Request[sNAME] != null )
						{
							...
							// 02/08/2009   We are not going to support images on the WebToLeadCapture page. 
							/*
							DynamicControl ctlCustomField_File = new DynamicControl(ctlPARENT, sNAME + "_File");
							// 04/21/2006   If the type is Guid and it is accompanied by a File control, then assume it is an image. 
							if ( sCsType == "Guid" && ctlCustomField.Type == "HtmlInputHidden" && ctlCustomField_File.Exists )
		...
		// 05/25/2008   We need a version of UpdateCustomFields that pulls data from a DataRow as this is how 
					...
					foreach(DataRow row in dtCustomFields.Rows)
					{
						...
						if ( rowForm.Table.Columns.Contains(sNAME) )
						{
							...
							switch ( sCsType )
							{
								...
								case "decimal" :  Sql.AddParameter(cmd, "@" + sNAME, Sql.ToDecimal (rowForm[sNAME]));  break;
								case "bool"    :  Sql.AddParameter(cmd, "@" + sNAME, Sql.ToBoolean (rowForm[sNAME]));  break;
								case "DateTime":  Sql.AddParameter(cmd, "@" + sNAME, Sql.ToDateTime(rowForm[sNAME]));  break;
		...
		// 12/29/2007   TEAM_ID is now in the stored procedure. 
		/*
		...
		// 11/30/2006   We need to be able to update the team when importing data. 
		public static void UpdateTeam(IDbTransaction trn, Guid gID, string sMODULE, Guid gTEAM_ID)
		{
			// 11/22/2006   Team management is optional. 

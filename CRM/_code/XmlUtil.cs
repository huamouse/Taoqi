...
namespace Taoqi
{
	...
	public class XmlUtil
	{
		...
		// 03/29/2008   We need a safe way to get the attribute. 
		...
		public static string SelectSingleNode(XmlNode parent, string sNode, XmlNamespaceManager nsmgr)
		{
			try
			{
				...
				{
					XmlNode node = null;
					// 10/24/2007   We need to support multiple tags. 
					string[] aNode = sNode.Split('/');
					int i = 0;
		...
		// 12/10/2009   We need to be able to set the attribute with a prefix. 
		...
		public static void Dump(ref StringBuilder sb, string sIndent, XmlNode parent)
		{
			...
			if ( parent.HasChildNodes )
			{
				if ( parent.ChildNodes.Count == 1 && parent.ChildNodes[0].NodeType == XmlNodeType.Text )
				{
					...
					{
						// 10/12/2006   Reduce the XML dump. 
						if ( child.Value.IndexOf(' ') > 0 )
						{
							sb.AppendLine(">");
							// 07/15/2010   Use tab to make it easier to format. 
		...
		public static string ConvertFromPHP(string sPHP)
		{
			...
			try
			{
				// 01/28/2009   Check for empty string before attempting to convert. 
		...
		public static string EncaseXpathString(string input)
		{
			...
			// 01/31/2010   Original code did not add the last part. 

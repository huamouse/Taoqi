...
namespace Taoqi
{
	...
	public class CreateItemTemplateACL : ITemplate
	{
		...
		private int NormalizeAccessValue(string sACCESS_TYPE, int nACCESS)
		{
			...
			{
				// 04/25/2006   Be flexible with the values, so don't compare directly to 89 and -98.
				if ( nACCESS > 0 )
					nACCESS = 89;
				else
					nACCESS = -98;
			...
			{
				// 04/25/2006   Be flexible with the values, so don't compare directly to 90 and -99.
				if ( nACCESS > 0 )
					nACCESS = 90;
				else
					nACCESS = -99;
			...
			{
				// 04/25/2006   Be flexible with the values, so don't compare directly to 90, 75 and -99.
				if ( nACCESS > 75 )
					nACCESS = 90;
				else if ( nACCESS > 0 )
					nACCESS = 75;
		...
		public void InstantiateIn(Control objContainer)
		{
			// 04/25/2006   The label needs to be created first as the List will need to access it. 
		...
		private void lit_OnDataBinding(object sender, EventArgs e)
		{
			...
			// 04/25/2006   We don't have access to the ACLGrid in InstantiateIn(), so do so here. 
			ACLGrid grd = objContainer.Parent.Parent as ACLGrid;
			if ( !grd.EnableACLEditing )
			{
				divList.Controls.Clear();
				// 04/25/2006   I'd like to remove the dvList, but I can't do it here. 
			...
			if ( row != null )
			{
				...
				// 04/25/2006   We could use GUIDS like SugarCRM, but since there is only one ACL grid, 
				...
				if ( row[sDATA_FIELD] != DBNull.Value )
				{
					...
					// 04/25/2006   Don't update values on postback, otherwise it will over-write modified values. 
					// 04/26/2006   Make sure to set the division color, even on postback. 
					// 05/03/2006   If we are not editing, then always bind to the query. 
					...
					{
						// 04/25/2006   The label will not retain its value, so restore from the hidden field.
						// We use the hidden field because the value my have been changed by the user. 
						// 04/25/2006   We are too early in the ASP.NET lifecycle to access hid.Value,
		...
		private void lst_OnDataBinding(object sender, EventArgs e)
		{
			...
			if ( row != null )
			{
				...
				// 04/25/2006   We could use GUIDS like SugarCRM, but since there is only one ACL grid, 
				// there is no need for that kind of uniqueness. 
				// 04/25/2006   toggleDisplay() will automatically reverse toggle of a linked control. 
				divList.ID = sMODULE_NAME + "_" + sACCESS_TYPE;
				lst.ID = "lst" + sMODULE_NAME + "_" + sACCESS_TYPE;
				// 04/25/2006   OnChange, update the the innerHTML and the hidden field. 
				...
				// 04/25/2006   The first parent is the DIV, the second is the TableCell. 
				...
				// 04/25/2006   We always need to translate the items, even during postback.
				...
				// 04/25/2006   Make sure to translate the text.  
	...
	public class ACLGrid : SplendidGrid
	{
		...
		// 04/25/2006   FindControl needs to be executed on the DataGridItem.  I'm not sure why.

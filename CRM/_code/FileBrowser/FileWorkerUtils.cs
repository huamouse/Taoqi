...
namespace Taoqi.FileBrowser
{
	public class FileWorkerUtils
	{
		// 11/06/2010   Move LoadFile() to Crm.EmailImages. 

		public static void LoadImage(ref Guid gImageID, ref string sFILENAME, IDbTransaction trn)
		{
			// 04/26/2012   CKEditor change the name to upload. 
			LoadImage(ref gImageID, ref sFILENAME, "upload", trn);
		}

		// 08/09/2009   We need to allow the field name to be a parameter so that this code can be reused. 

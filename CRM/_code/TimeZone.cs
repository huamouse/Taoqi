...
namespace Taoqi
{
	...
	public class TimeZone
	{
		...
		// 09/25/2010   We need access to the time zone properties in ReportService2010.asmx. 
		...
		// 06/26/2010   We need to be able to create a timezone from within the Workflow engine. 
		public static TimeZone CreateTimeZone(HttpApplicationState Application, Guid gTIMEZONE)
		{
			TimeZone T10z = Application["TIMEZONE." + gTIMEZONE.ToString()] as Taoqi.TimeZone;
			if ( T10z == null )
			{
				// 08/29/2005  First try and use the default from CONFIG. 
				gTIMEZONE = Sql.ToGuid(Application["CONFIG.default_timezone"]);
				T10z = Application["TIMEZONE." + gTIMEZONE.ToString()] as Taoqi.TimeZone;
				if ( T10z == null )
				{
				...
				if ( T10z == null )
				{
					...
					// T10z = new TimeZone();
					// 07/25/2006    Still having a problem with the hosting company.  
		...
		public TimeZone()
		{
			...
			{
				// 07/25/2006   Some web hosting companies have tight security and block all access to the registry. 
		...
		public DateTime FromServerTime(object objServerTime)
		{
			...
			if ( dtServerTime == DateTime.MinValue )
				return dtServerTime;
			// 03/25/2013   Force the kind so that ToUniversalTime() will work. 
			if ( dtServerTime.Kind != DateTimeKind.Local )
				dtServerTime = DateTime.SpecifyKind(dtServerTime, DateTimeKind.Local);
			// 11/07/2005   SugarCRM 3.5 now stores time in GMT.
            //if (m_bGMTStorage)
		...
		{
			// 11/29/2008   The time cannot be converted if it is already ad the min value. 
			if ( dtServerTime == DateTime.MinValue )
				return dtServerTime;
			// 03/25/2013   Force the kind so that ToUniversalTime() will work. 
			if ( dtServerTime.Kind != DateTimeKind.Local )
		...
		{
			// 11/07/2005   Don't modify if value is MinValue.
		...
		// 04/04/2006   SOAP needs a quick way to convert from UniversalTime to ServerTime. 
		public DateTime ToServerTimeFromUniversalTime(DateTime dtUniversalTime)
		{
			if ( dtUniversalTime == DateTime.MinValue )
				return dtUniversalTime;
			// 03/25/2013   Force the kind so that ToLocalTime() will work. 
		...
		// 08/17/2006   SOAP needs a quick way to convert from ServerTime to UniversalTime. 

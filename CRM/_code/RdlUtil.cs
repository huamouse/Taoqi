...
namespace Taoqi
{
	public class RdlDocument : XmlDocument
	{
		...
		// 06/20/2006   Use the 2005 spec as it has better support for custom properties. 
		...
		{
			// 09/26/2010   Add support for ReportBuilder 3.0 files. 
			if ( sDefaultNamespace == "http://schemas.microsoft.com/sqlserver/reporting/2010/01/reportdefinition" )
			{
		...
		{
			// 09/27/2010   The current version of the ReportBuilder does not support the 2010/01 schema. 
			...
			if( xBody != null )
			{
				...
				foreach ( XmlNode xDataSet in xDataSets )
				{
					...
					{
						// 07/06/2011   We need to update the data source for any existing query. 
						// The other option is to change the name of the data source to dataSource1. 
						// The dataset 'vwLEADS' refers to the data source 'Taoqi', which does not exist. 
						XmlUtil.SetSingleNode(xml, xQuery, "DataSourceName", "dataSource1", nsmgr, sDefaultNamespace);
					}
...
//			xml.Save("C:\\Temp\\RDL2008.rdl.xml");
		...
		{
			if ( rdl.IndexOf("http://schemas.microsoft.com/sqlserver/reporting/2010/01/reportdefinition") > 0 )
			{
				rdl = ConvertRDL2010ToRDL2008(rdl);
			}

			base.LoadXml(rdl);
			// 01/24/2010   The Report tag does not support the Name attribute. 
			...
			// 06/20/2006   The default namespace cannot be selected, so create an alias and reference the alias. 
			// 12/08/2009   If this is a 2008 schema, then specify the 2008 namespace. 
			else if ( rdl.IndexOf("http://schemas.microsoft.com/sqlserver/reporting/2008/01/reportdefinition") > 0 )
			{
				// 01/24/2010   In order to properly process ReportBuilder 2.0 report parameters, 
				...
				nsmgr.AddNamespace("defaultns", sDefaultNamespace );
				// 01/12/2012   Report Builder 3.0 is having an issue with the width and height. 
			...
			// 06/22/2006   The designer namespace must be manually added. 
			nsmgr.AddNamespace("rd", sDesignerNamespace);
			
			bool bValidate = false;
#if DEBUG
			// 09/28/2010   The Workflow RDL will fail validation because we do not create a full document. 
			//bValidate = true;
#endif
			if ( bValidate )
			{
		...
		// 11/20/2011   Charting needs a way to skip updating if value exists. 
		...
		public RdlDocument(string sNAME) : base()
		{
			...
			// 06/20/2006   Add the RD namespace manually. 
			...
			// 06/20/2006   The default namespace cannot be selected, so create an alias and reference the alias. 
			nsmgr.AddNamespace("defaultns", sDefaultNamespace );
			// 02/11/2010   The Report Name is important, so store in the Custom area. 
		...
		public RdlDocument(string sNAME, string sAUTHOR, bool bChart) : base()
		{
			...
			// 06/18/2006   The report definition element 'Report' is empty at line 14, position 5054. It is missing a mandatory child element of type 'Width'. 
			...
			if ( !bChart )
			{
				...
				// 06/21/2006   TableRow requires a Height element. 
				...
				// 06/21/2006   Table requires a TableColumns element. 
				SetSingleNode         ("Body/ReportItems/Table/TableColumns"                         , String.Empty);
			}
			// 06/21/2006   Body requires a Height element. 
			SetSingleNode         ("Body/Height"                                                 , "8.5in"      );
			
			// 06/21/2006   If PageFooter is included, then it must have a Height element. 
		...
		public void SetCustomProperty(string sName, string sValue)
		{
			...
			// 05/27/2006   All Taoqi properties should start with "crm". 
					...
					if ( iDetail < nlHeaderValues.Count )
					{
						...
						try
						{
							...
							if ( sDetail.StartsWith("=Fields!") && sDetail.EndsWith(".Value") )
							{
								...
								// 06/18/2006   Now translate the field name to the column name. 
		...
		// 07/12/2006   The textbox name needs to be derived from the field name.
		...
		public void CreateTextboxValue(XmlNode parent, string sTextboxName, string sValue, bool bField)
		{
			...
			if ( bField )
			{
				// 06/21/2006   Textbox requires a Name attribute. 
				this.SetSingleNodeAttribute(xTextbox, "Name", RdlName(sTextboxName) + "__Value");
				// 06/18/2006   Field names must be CLS-compliant identifiers. Use two underscores to distinguish betwen typical use of underscore. 
		...
		public void UpdateDataTable(DataTable dtDisplayColumns)
		{
			...
			if ( dtDisplayColumns != null )
			{
				...
				if ( dtDisplayColumns.Rows.Count > 0 )
				{
					...
					// 11/24/2006   The ReportViewer only understands the en-US decimal format. 
					CultureInfo ciEnglish = CultureInfo.CreateSpecificCulture("en-US");
					sWidth = dWidth.ToString("#.##", ciEnglish.NumberFormat) + "in";
				}
		...
		// 11/06/2011   MS Charts do the grouping after the selected.  We will convert the select to do the grouping. 
		public void BuildChartCommand(HttpContext Context, XmlNode xDataSet, IDbCommand cmd)
		{
			...
			// 01/30/2012   Make sure to set the command type if it is a stored procedure. 
		...
		public void UpdateChart(string sChartTitle, string sChartType, string sSeriesTitle, string sSeriesColumn, string sSeriesOperator, string sCategoryTitle, string sCategoryColumn, string sCategoryOperator, string sDateFormat)
		{
			...
			// 11/06/2011   Remove all existing report items. 
			// 11/12/2011   Don't remove existing items.  Just modify the first chart so that Report Builder changes will not get thrown away. 
			...
			// 03/12/2012   The chart needs the Page tags in order to be a valid RDL. 
		...
		{
			// 07/13/2006   The field name now always starts with @ and ends with __00A, with the last two digits being the parameter index. 
			if ( sName.StartsWith("@") )
			{
		...
		{
			// 07/13/2006   The ReportViewer does not know how to translate the date functions. 
			...
			// 10/06/2012   Move the DataSourceReference fix to ReportViewerFixups.
			XmlNodeList nlDataSets = this.SelectNodesNS("DataSets/DataSet");
			foreach ( XmlNode xDataSet in nlDataSets )
			{
				string sDataSetName = xDataSet.Attributes.GetNamedItem("Name").Value;
				// 12/21/2009   The new VS 2010 ReportViewer does not like the DataSourceReference.  
				...
				if ( xDataSource != null )
				{
					...
					if ( xDataSourceReference != null )
					{
						...
						/*
						// 01/21/2010   This loop did not remove all nodes.  Use RemoveAll() instead. 
						...
						*/
						...
						// 01/24/2010   RemoveAll will also remove the Name attribute, which is required. 
		...
		// 11/16/2008   DbSpecificDate is only called from the workflow engine, so it is safe to convert TODAY() for SQL Server. 
		public static string DbSpecificDate(string sSplendidProvider, string sValue)
		{
			...
			{
				// 11/20/2008   SQL Server does not define the TODAY() function, so we must translate. 
				switch ( sValue )
				{
		...
		{
			// 03/14/2011   Oracle can have date formatting issues, so force a .NET friendly date format. 
			if ( Sql.IsOracle(cmd) )
			{
				// 03/14/2011   Make sure not to include a trailing semicolon, otherwise we will get the error: ORA-00911: invalid character. 
				cmd.CommandText = "ALTER SESSION SET NLS_DATE_FORMAT='YYYY-MM-DD HH24:MI:SS'";
				cmd.ExecuteNonQuery();
			}
			cmd.CommandText = XmlUtil.SelectSingleNode(xDataSet, "Query/CommandText", nsmgr);
			...
			// 07/12/2006   If not SQL Server and not Sybase, then we must remove the DBO. 
			if ( !Sql.IsSQLServer(cmd) && !Sql.IsSybase(cmd) )
			{
				cmd.CommandText = cmd.CommandText.Replace(" dbo.fnDateOnly("   , " fnDateOnly("   );
				cmd.CommandText = cmd.CommandText.Replace(" dbo.fnMonthOnly("  , " fnMonthOnly("  );
			...
			// 04/17/2007   @MEMBERSHIP_USER_ID is a special replacement field that we use to filter results based on ACL rules. 
			// 03/14/2011   The special Membership User ID needs to be replaced earlier. 
			//cmd.CommandText = cmd.CommandText.Replace("@MEMBERSHIP_USER_ID", "'" + Security.USER_ID.ToString() + "'");
			// 03/14/2011   We should use a parameter. 
			if ( cmd.CommandText.Contains("@MEMBERSHIP_USER_ID") )
				Sql.AddParameter(cmd, "@MEMBERSHIP_USER_ID", Security.USER_ID);
		...
		public void BuildCommandQueryParameter(XmlNode xDataSet, XmlNode xQueryParameter, IDbCommand cmd)
		{
			...
			if ( sValue.StartsWith("Parameters!") && sValue.EndsWith(".Value") )
			{
				...
				if ( xReportParameterValue != null )
				{
					...
					// 10/24/2007   The ReportViewer will complain if the query parameter does not have a value. 
					this.SetSingleNode(xQueryParameter, "Value", sValue);
					// 03/04/2012   If there are multiple values, then we need to parse the SQL manually. 
			...
			// 09/26/2010   ReportBuilder 3.0 does not always include the System. in the type name. 
			...
			switch ( sTypeName )
			{
				...
				default        :
					// 05/21/2012   A date parameter type will be string, so we need another way to detect that the type is a date. 
	...
	public class RdsDocument : XmlDocument
	{
		...
		public RdsDocument(string sNAME) : base()
		{
			...
			// 09/25/2010   Add the RD namespace manually. 
			XmlUtil.SetSingleNodeAttribute(this, this.DocumentElement, "xmlns:rd", sDesignerNamespace);

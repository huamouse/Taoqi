...
namespace Taoqi
{
	...
	public class SplendidImport
	{
		...
		public static void Import(XmlDocument xml, ArrayList arrTables, bool bTruncate)
		{
			...
			// 12/16/2005   First create a hash table to convert tab name to a uppercase table name. 
			...
			// 09/29/2006   The following are SugarCRM config tables. 
			lstReservedTables.Add("ACL_ACTIONS"              );
			lstReservedTables.Add("CURRENCIES"               );
			lstReservedTables.Add("RELATIONSHIPS"            );
			// 10/01/2006   Can't reserve Users because too many other tables depend on it. 
			...
			if ( arrTables == null )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						// 10/02/2006   We need to delete tables that reference the tables we are importing. 
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								using ( DataTable dt = new DataTable() )
								{
									...
									for ( int i = 0 ; i < dt.Rows.Count && Response.IsClientConnected ; i++ )
									{
										...
										// 12/18/2005   Some tables are reserved and should not be truncated or imported. 
										if ( lstReservedTables.Contains(sTABLE_NAME) )
											continue;
										// 12/18/2005   Only truncate tables that are being imported or a table it references. 
										// 10/02/2006   We need to truncated referenced tables because they may reference a table being imported. 
										if ( hashTables.ContainsKey(sTABLE_NAME) || hashReferenced.ContainsKey(sTABLE_NAME) )
										{
											try
											{
												...
												{
													// 12/17/2005   Don't delete the existing user, otherwise it will cause a login problem in the future. 
													cmd.CommandText = "delete from USERS where ID != @ID";
													Sql.AddParameter(cmd, "@ID", Security.USER_ID);
												}
												else
												{
													// 05/04/2008   Protect against SQL Injection. A table name will never have a space character.
			...
			for ( int i = 0 ; i < arrTables.Count && Response.IsClientConnected ; i++ )
			{
				...
				if ( hashTables.ContainsKey(sTABLE_NAME) )
				{
					...
					if ( nlRows.Count > 0 )
					{
						...
						// 12/17/2005   Use a new connection for each table import so that connection state will be reset.  
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							try
							{
								// 12/08/2009   Identity fields have been replaced with the NUMBER_SEQUENCES table. 
								/*
								if ( Sql.IsSQLServer(con) )
								{
									// 12/17/2005   In SQL Server, turn on identity_insert. 
									string sIDENTITY_NAME = String.Empty;
									// 09/30/2006   Switch needs to use sTABLE_NAME and not sIDENTITY_NAME. 
									switch ( sTABLE_NAME )
									{
								...
								{
									// 12/17/2005   In Oracle, disable sequence triggers. 
								...
								*/
								...
								using ( DataTable dtColumns = new DataTable() )
								{
									...
									foreach(XmlNode node in nlRows)
									{
										...
										// 09/28/2006   Always start with a blank command and only insert fields that are used.  
										using ( IDbCommand cmdImport = con.CreateCommand() )
										{
											cmdImport.CommandText = "";
											cmdImport.CommandType = CommandType.Text;
											// 09/30/2006   Count row errors so that we can skip bad rows, but continue with the rest of the import. 
											int nRowErrors = 0;
											// 09/28/2006   Build the insert statement using only the fields provided in the data. 
											...
											for ( int j = 0; j < node.ChildNodes.Count; j++ )
											{
												...
												if ( vwColumns.Count == 1 )
												{
													...
													// 09/28/2006   If the field is specified twice, then the second instance will prevail. 
													if ( Sql.FindParameter(cmdImport, "@" + sName) == null )
													{
														if ( sbFields.Length > 0 )
															sbFields.Append(", ");
														sbFields.Append(sName);
														// 09/28/2006   We cannot use a StringBuilder for the values because we need to allow 
													...
													// 12/18/2005   A short-sighted programmer at SugarCRM created GUIDs with invalid characters. 
													// We need to convert them to valid GUIDs. 
													string sText = node.ChildNodes[j].InnerText;
													// 08/20/2006   Dynamically attempt to fix invalid GUIDs. It really only works for the ones defined below. 
											...
											// 10/01/2006   Some tables in SugarCRM do not have an ID.  If Taoqi has an ID, then it is likely required. 
											// 10/01/2006   We only need to specify the ID if the platform is not SQL Server.
											...
											if ( nRowErrors == 0 )
											{
												...
												catch(Exception ex)
												{
													...
													// 12/17/2005   If there is an error, stop importing from this table. 
													// 12/18/2005   I'd like to see the first 100 errors. 
													nTableErrors++ ;
													if ( nTableErrors > 100 )
														break;
												}
							...
							finally
							{
								try
								{
									...
									if ( Sql.IsSQLServer(con) )
									{
										// 12/17/2005   In SQL Server, turn off identity_insert. 
										string sIDENTITY_NAME = String.Empty;
										// 09/30/2006   Switch needs to use sTABLE_NAME and not sIDENTITY_NAME. 
										switch ( sTABLE_NAME )
										{
									...
									{
										// 12/17/2005   In Oracle, enable sequence triggers. 
										string sTRIGGER_NAME = String.Empty;
										switch ( sTABLE_NAME )
										{
											case "BUGS"          : sTRIGGER_NAME = "TR_S_BUGS_BUG_NUMBER"       ;  break;
			...
			// 12/18/2005   Reserved tables will still be imported, but we use the associated spXXX_Update procedure.
			for ( int i = 0 ; i < arrTables.Count && Response.IsClientConnected ; i++ )
			{
				string sTABLE_NAME = arrTables[i].ToString().ToUpper();
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							try
							{
								...
								foreach(XmlNode node in nlRows)
								{
									...
									// 10/10/2006   Use IDbDataParameter to be consistent. 
									foreach(IDbDataParameter par in cmdImport.Parameters)
									{
										...
										if ( sParameterName == "TEAM_ID" && bEnableTeamManagement ) // 02/26/2011   Ignore the Required flag. && bRequireTeamManagement )
											par.Value = Sql.ToDBGuid(Security.TEAM_ID);  // 02/26/2011   Make sure to convert Guid.Empty to DBNull. 
										else if ( sParameterName == "ASSIGNED_USER_ID" ) // 02/26/2011   Always set the Assigned User ID. && bRequireUserAssignment )
									...
									for ( int j = 0; j < node.ChildNodes.Count; j++ )
									{
										...
										if ( (sName == "ID" || sName.EndsWith("_ID") || sName == "CREATED_BY") )
										{
											// 10/05/2006   IDs must be in upper case.  This is primarily for platforms that are case-significant. 
											sText = sText.ToUpper();
											if ( sText.Length < 36 && sText.Length > 0 )
											{
												sText = "00000000-0000-0000-0000-000000000000".Substring(0, 36 - sText.Length) + sText;
		...
		public static void ZipSaveStream(Stream stm, string sTempPathName)
		{
			using ( FileStream stmDatabase = new FileStream(sTempPathName, FileMode.Create) )
			{
				using ( BinaryWriter mwtr = new BinaryWriter(stmDatabase) )
				{
					...
					// 01/31/2010   When working with a ZipStream, it is better not to use a StreamReader. 
					while ( true )
					{
		...
		public static XmlDocument ConvertTableToXml(DataTable dt, string sRecordName, bool bWithHeader, bool bUseColumnName, List<String> arrIgnoreColumns)
		{
			...
			// 01/30/2010   Add ability to include table column names as the header for dBase import. 
			if ( bWithHeader )
			{
		...
		public static XmlDocument ConvertDBaseToXml(Stream stm, string sRecordName, bool bUseColumnName, List<String> arrIgnoreColumns, List<String> arrAdditionalColumns)
		{
			...
			// 01/31/2010   Due to the 8 character dBase limitation, we should create our own folder. 
			...
			// 01/30/2010   The ODBC driver seems to have an 8 character file name limit. 
			string sTempFileName = sTempFileID.Replace("-", "").Substring(0, 8) + ".dbf";
			string sTempPathName = Path.Combine(sTempPath, sTempFileName);
			// 01/30/2010   We should make sure that the file name is unique. 
				...
				using ( OdbcCommand cmd = con.CreateCommand() )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						try
						{
							...
							// 02/02/2010   We need the ability to add empty fields. 
							...
							// 02/02/2010   Lets remove the column before the conversion to XML. 
		...
		public static XmlDocument ConvertXmlSpreadsheetToXml(XmlDocument xml, string sRecordName)
		{
			...
			// 08/22/2006   The Spreadsheet namespace is also the default namespace, so make sure to prefix nodes with ss.
			...
			if ( xWorksheet != null )
			{
				...
				if ( xTable != null )
				{
					...
					if ( nlRows.Count > 0 )
					{
						// 08/22/2006   The first row is special in that we must make sure that all nodes exist. 
						...
						for ( int i = 0; i < nlRows.Count; i++ )
						{
							...
							for ( int j = 0, nField = 0; j < xRow.ChildNodes.Count; j++, nField++ )
							{
								...
								// 08/22/2006   If there are any missing cells, then add them.

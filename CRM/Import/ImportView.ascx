<%@ Control CodeBehind="ImportView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="Taoqi.Import.ImportView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divImportView">
	...
	<Taoqi:ModuleHeader ID="ctlModuleHeader" Module="Import" Title="Import.LBL_MODULE_NAME" EnableModuleLabel="false" EnablePrint="false" HelpName="ImportView" EnableHelp="true" Runat="Server" />
	<p>
	...
	function SelectSourceFormat()
	{
		...
		document.getElementById('spnStep3Upload'  ).style.display = (!(bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks) ? 'inline' : 'none');
		document.getElementById('spnStep3Connect' ).style.display = ( (bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks) ? 'inline' : 'none');
		document.getElementById('tblUploadFile'   ).style.display = (!(bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks) ? 'inline' : 'none');
		document.getElementById('tblUploadConnect').style.display = ( (bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks) ? 'inline' : 'none');
	...
	function MoveLeftToRight(sLeftID, sRightID, bReverse)
	{
		...
		for ( i=lstLeft.options.length-1; i >= 0  ; i-- )
		{
			if ( lstLeft.options[i].selected == true )
			{
				// 10/11/2006   Firefox does not support options.remove(), so just set the option to null. 
				lstLeft.options[i] = null;
			}
		}
		// 08/05/2005  Don't use the sLeftID & sRightID values as they can be reversed. 
		...
		<asp:Table SkinID="tabForm" runat="server">
			<asp:TableRow>
				<asp:TableCell>
						...
						<tr>
							<td width="35%" class="dataField">
								<nobr>
									...
									<hr />
									<asp:RadioButton ID="radLINKEDIN"         GroupName="radSOURCE" Text='<%# L10n.Term("Import.LBL_LINKEDIN"        ) %>' CssClass="checkbox" OnCheckedChanged="SOURCE_TYPE_CheckedChanged" AutoPostBack="true" Enabled='<%# !Sql.IsEmptyString(Application["CONFIG.LinkedIn.APIKey"            ]) %>' runat="server" /><br />
					...
					<Taoqi:ListHeader Visible="<%# !PrintView %>" Title="Import.LBL_MY_SAVED" Runat="Server" />
					<Taoqi:SplendidGrid id="grdMySaved" AllowPaging="false" AllowSorting="false" EnableViewState="true" runat="server">
						<Columns>
							...
							<asp:TemplateColumn HeaderText="" ItemStyle-HorizontalAlign="Right" ItemStyle-Wrap="false">
								<ItemTemplate>
									<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
										<asp:ImageButton CommandName="Import.Delete" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_DELETE") %>' SkinID="delete_inline" Runat="server" />
	...
	<div id="divImportStep5" style="DISPLAY: none">
		<asp:Table SkinID="tabForm" runat="server">
			<asp:TableRow>
				<asp:TableCell>
					<%@ Register TagPrefix="Taoqi" Tagname="Chooser" Src="~/_controls/Chooser.ascx" %>
					<Taoqi:Chooser ID="ctlDuplicateFilterChooser" ChooserTitle="Import.LBL_DUPLICATE_FILTER" LeftTitle="Import.LBL_FILTER_COLUMNS" RightTitle="Import.LBL_AVAILABLE_COLUMNS" Enabled="true" Runat="Server" />
				</asp:TableCell>
			</asp:TableRow>
		</asp:Table>
	</div>
					...
					<asp:Table SkinID="tabEditView" runat="server">
						<asp:TableRow>
							...
							<asp:TableCell VerticalAlign="top" Visible="false">
								...
								<script runat="server">
									// 10/25/2010   You have to be careful with Reevaluation Always as it will re-evaluate 
		...
		<asp:Table SkinID="tabForm" runat="server">
			<asp:TableRow>
				<asp:TableCell>
					...
					<br />
					<Taoqi:ListHeader ID="ctlListHeader" Visible="<%# !PrintView %>" Title="Import.LBL_LAST_IMPORTED" Runat="Server" />
					<Taoqi:SplendidGrid id="grdMain" AllowPaging="<%# !PrintView %>" AllowSorting="true" EnableViewState="true" runat="server">
					</Taoqi:SplendidGrid>
				</asp:TableCell>
			</asp:TableRow>
		</asp:Table>
